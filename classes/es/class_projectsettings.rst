...
Este directorio de usuario se utiliza para almacenar datos persistentes (sistema de archivos ``user://``). Si se deja vacío, ``user://`` se resuelve en una carpeta específica del proyecto en la propia carpeta de configuración de Godot (véase :ref:`OS.get_user_data_dir<class_OS_method_get_user_data_dir>`). Si se define un nombre de directorio personalizado, este nombre se utilizará en su lugar y se añadirá al directorio de datos de usuario específico del sistema (la misma carpeta principal que la carpeta de configuración de Godot documentada en :ref:`OS.get_user_data_dir<class_OS_method_get_user_data_dir>`).

El ajuste :ref:`application/config/use_custom_user_dir<class_ProjectSettings_property_application/config/use_custom_user_dir>` debe estar activado para que esto surta efecto.
...
Si ``true``, el proyecto guardará los datos de los usuarios en su propio directorio de usuarios (véase :ref:`application/config/custom_user_dir_name<class_ProjectSettings_property_application/config/custom_user_dir_name>`). Esta configuración sólo es efectiva en las plataformas de escritorio. Se debe establecer un nombre en el parámetro :ref:`application/config/custom_user_dir_name<class_ProjectSettings_property_application/config/custom_user_dir_name>` para que esto tenga efecto. Si ``false``, el proyecto guardará los datos del usuario en ``(directorio de datos de usuario del sistema operativo)/Godot/app_userdata/(nombre del proyecto)``.
...
Archivo de recursos :ref:`AudioBusLayout<class_AudioBusLayout>` por defecto para usar en el proyecto, a menos que sea sobreescrito por la escena.
...
Sobreescritura más segura para :ref:`audio/output_latency<class_ProjectSettings_property_audio/output_latency>` en la plataforma web, para evitar problemas de audio especialmente en los dispositivos móviles.
...
Habilita `la coincidencia a larga distancia <https://github.com/facebook/zstd/releases/tag/v1.3.2>`__ en Zstandard.
...
Si ``true``, habilita advertencias cuando se llama a una función sin utilizar su valor de retorno (asignándolo a una variable o utilizándolo como argumento de la función). Esos valores de retorno se utilizan a veces para denotar posibles errores mediante el uso de enum :ref:`Error<enum_@GlobalScope_Error>`.
...
Valor por defecto para :ref:`ScrollContainer.scroll_deadzone<class_ScrollContainer_property_scroll_deadzone>`, que se utilizará para todos los :ref:`ScrollContainer<class_ScrollContainer>`\ s a menos que se sobrescriba.
...
Ruta a un archivo de recursos :ref:`Theme<class_Theme>` personalizado para usar en el proyecto (``theme`` o genérico ``tres``/``res`` extension).
...
Ruta a un recurso :ref:`Font<class_Font>` personalizado para usar como predeterminado para todos los elementos de la interfaz gráfica de usuario del proyecto.
...
Ajuste del temporizador para la búsqueda incremental en los controles de :ref:`Tree<class_Tree>`, :ref:`ItemList<class_ItemList>`, etc. (en milisegundos).
...
Temporizador para detectar la inactividad en :ref:`TextEdit<class_TextEdit>` (en segundos).
...
:ref:`InputEventAction<class_InputEventAction>` por defecto para confirmar un botón enfocado, un menú o un elemento de la lista, o validar la entrada.

\ **Nota:** Las acciones ``ui_*`` por defecto no pueden ser eliminadas ya que son necesarias para la lógica interna de varios :ref:`Control<class_Control>`\ s. Sin embargo, los eventos asignados a la acción pueden ser modificados.
...
:ref:`InputEventAction<class_InputEventAction>` por defecto para descartar una entrada modal o pendiente.
...
:ref:`InputEventAction<class_InputEventAction>` por defecto para moverse hacia abajo en la UI.
...
:ref:`InputEventAction<class_InputEventAction>` por defecto para ir a la posición final de un :ref:`Control<class_Control>` (por ejemplo, el último elemento de una :ref:`ItemList<class_ItemList>` o de un :ref:`Tree<class_Tree>`), que coincide con el comportamiento de la :ref:`@GlobalScope.KEY_END<class_@GlobalScope_constant_KEY_END>` en los típicos sistemas de interfaz de usuario de escritorio.

\ **Nota:** Las acciones predeterminadas de ``ui_*`` no se pueden eliminar ya que son necesarias para la lógica interna de varios :ref:`Control<class_Control>`\ s. Sin embargo, los eventos asignados a la acción pueden ser modificados.
...
:ref:`InputEventAction<class_InputEventAction>` por defecto para enfocar el siguiente :ref:`Control<class_Control>` en la escena. El comportamiento de enfoque puede ser configurado a través del :ref:`Control.focus_next<class_Control_property_focus_next>`.

\ **Nota:** Las acciones por defecto de ``ui_*`` no se pueden eliminar ya que son necesarias para la lógica interna de varios :ref:`Control<class_Control>`\ s. Sin embargo, los eventos asignados a la acción pueden ser modificados.
...
:ref:`InputEventAction<class_InputEventAction>` por defecto para enfocar el :ref:`Control<class_Control>` anterior en la escena. El comportamiento de enfoque puede ser configurado a través del :ref:`Control.focus_previous<class_Control_property_focus_previous>`.
...
:ref:`InputEventAction<class_InputEventAction>` por defecto para ir a la posición de inicio de un :ref:`Control<class_Control>` (por ejemplo, el primer elemento de una :ref:`ItemList<class_ItemList>` o de un :ref:`Tree<class_Tree>`), que coincide con el comportamiento de :ref:`@GlobalScope.KEY_HOME<class_@GlobalScope_constant_KEY_HOME>` en los típicos sistemas de interfaz de usuario de escritorio.
...
:ref:`InputEventAction<class_InputEventAction>` por defecto para moverse a la izquierda en la UI.
...
:ref:`InputEventAction<class_InputEventAction>` por defecto para bajar una página en un :ref:`Control<class_Control>` (por ejemplo, en una :ref:`ItemList<class_ItemList>` o un :ref:`Tree<class_Tree>`), coincidiendo con el comportamiento de :ref:`@GlobalScope.KEY_PAGEDOWN<class_@GlobalScope_constant_KEY_PAGEDOWN>` en los típicos sistemas de interfaz de usuario de escritorio.
...
:ref:`InputEventAction<class_InputEventAction>` por defecto para subir una página en un :ref:`Control<class_Control>` (por ejemplo, en una :ref:`ItemList<class_ItemList>` o en un :ref:`Tree<class_Tree>`), que coincida con el comportamiento de :ref:`@GlobalScope.KEY_PAGEUP<class_@GlobalScope_constant_KEY_PAGEUP>` en los típicos sistemas de interfaz de usuario de escritorio.
...
:ref:`InputEventAction<class_InputEventAction>` por defecto para moverse a la derecha en la UI.
...
:ref:`InputEventAction<class_InputEventAction>` por defecto para seleccionar un elemento en un :ref:`Control<class_Control>` (por ejemplo, en una :ref:`ItemList<class_ItemList>` o en un :ref:`Tree<class_Tree>`).

\ **Nota:** Las acciones de ``ui_*`` por defecto no se pueden eliminar ya que son necesarias para la lógica interna de varios :ref:`Control<class_Control>`\ s. Sin embargo, los eventos asignados a la acción pueden ser modificados.
...
:ref:`InputEventAction<class_InputEventAction>` por defecto para subir en la UI.
...
Tamaño máximo (en kiB) para el buffer de entrada :ref:`WebRTCDataChannel<class_WebRTCDataChannel>`.
...
El paquete de certificados de CA para usar en las conexiones SSL. Si se establece en un valor no vacío, esto *sobreescribira* el paquete de certificados de Godot por defecto `paquete de certificados de Mozilla <https://github.com/godotengine/godot/blob/master/thirdparty/certs/ca-certificates.crt>`__. Si se deja vacío, se utilizará el paquete de certificados por defecto.
...
Habilita :ref:`Viewport.physics_object_picking<class_Viewport_property_physics_object_picking>` en el viewport raíz.
...
Color de fondo claro por defecto. Sobrescribible por el :ref:`Viewport<class_Viewport>` usando su :ref:`Environment<class_Environment>`. Vea :ref:`Environment.background_mode<class_Environment_property_background_mode>` y :ref:`Environment.background_color<class_Environment_property_background_color>` en particular. Para cambiar este color predeterminado de forma programada, utilice el :ref:`VisualServer.set_default_clear_color<class_VisualServer_method_set_default_clear_color>`.
...
Sobrescritura del extremo inferior para :ref:`rendering/quality/depth/hdr<class_ProjectSettings_property_rendering/quality/depth/hdr>` en los dispositivos móviles, debido a problemas de rendimiento o de apoyo al driver.
...
Sobrescritura del extremo inferior para :ref:`rendering/quality/directional_shadow/size<class_ProjectSettings_property_rendering/quality/directional_shadow/size>` en los dispositivos móviles, debido a problemas de rendimiento o de apoyo al driver.
...
Anulación del extremo inferior para :ref:`rendering/quality/intended_usage/framebuffer_allocation<class_ProjectSettings_property_rendering/quality/intended_usage/framebuffer_allocation>` en los dispositivos móviles, debido a problemas de rendimiento o de compatibilidad con el controlador.
...
Sobreescritura del extremo inferior para :ref:`rendering/quality/reflections/texture_array_reflections<class_ProjectSettings_property_rendering/quality/reflections/texture_array_reflections>` en dispositivos móviles, debido a problemas de rendimiento o compatibilidad con el controlador.
...
Sobreescritura del extremo inferior para :ref:`rendering/quality/shading/force_blinn_over_ggx<class_ProjectSettings_property_rendering/quality/shading/force_blinn_over_ggx>` en los dispositivos móviles, debido a problemas de rendimiento o de soporte de los conductores.
...
Sobreescritura del extremo inferior para :ref:`rendering/quality/shading/force_lambert_over_burley<class_ProjectSettings_property_rendering/quality/shading/force_lambert_over_burley>` en los dispositivos móviles, debido a problemas de rendimiento o de apoyo al conductor.
...
Sobreescritura del extremo inferior para :ref:`rendering/quality/shading/force_vertex_shading<class_ProjectSettings_property_rendering/quality/shading/force_vertex_shading>` en los dispositivos móviles, debido a problemas de rendimiento o de compatibilidad con el controlador.
...
Anulación del extremo inferior para :ref:`rendering/quality/shadow_atlas/size<class_ProjectSettings_property_rendering/quality/shadow_atlas/size>` en los dispositivos móviles, debido a problemas de rendimiento o de compatibilidad con el controlador.
...
- opcionalmente ``hint``: :ref:`int<class_int>` (ver :ref:`PropertyHint<enum_@GlobalScope_PropertyHint>`) y ``hint_string``: :ref:`String<class_String>`\ 
