...
Si se crea un cono de ``true``, el :ref:`radius<class_CSGCylinder_property_radius>` sólo se aplicará a un lado.
...
Si ``true`` las normales del cilindro se ajustan para dar un efecto suave haciendo que el cilindro parezca redondeado. Si ``false`` el cilindro tendrá un aspecto de sombra plana.
