...
Devuelve un trozo de datos con los bytes recibidos. La cantidad de bytes a recibir puede solicitarse en el argumento ``bytes``. Si no hay suficientes bytes disponibles, la función se bloqueará hasta que se reciba la cantidad deseada. Esta función devuelve dos valores, un código :ref:`Error<enum_@GlobalScope_Error>` y una array de datos.
...
Devuelve un trozo de datos con los bytes recibidos. La cantidad de bytes a recibir puede ser solicitada en el argumento "bytes". Si no hay suficientes bytes disponibles, la función devolverá cuántos se han recibido realmente. Esta función devuelve dos valores, un código :ref:`Error<enum_@GlobalScope_Error>`, y una array de datos.
...
Obtiene una cadena con longitud de bytes ``bytes`` de la corriente. Si ``bytes`` es negativo (por defecto) la longitud se leerá de la corriente usando el proceso inverso de :ref:`put_string<class_StreamPeer_method_put_string>`.
...
Obtiene una cadena UTF-8 con longitud de bytes ``bytes`` del flujo (esto decodifica la cadena enviada como UTF-8). Si ``bytes`` es negativo (por defecto) la longitud será leída desde la corriente usando el proceso inverso de :ref:`put_utf8_string<class_StreamPeer_method_put_utf8_string>`.
...
Envía un trozo de datos a través de la conexión, bloqueando si es necesario hasta que se termine de enviar los datos. Esta función devuelve un código :ref:`Error<enum_@GlobalScope_Error>`.
...
Envía un trozo de datos a través de la conexión. Si no se pudieron enviar todos los datos a la vez, sólo una parte de ellos lo hará. Esta función devuelve dos valores, un código :ref:`Error<enum_@GlobalScope_Error>` y un entero, que describe cuántos datos se enviaron realmente.
