...
Calcola una funzione di Step all'interno del grafico di visual shader.
...
Tradotto come ``step(edge, x)`` nel linguaggio di shader.

Restituisce ``0.0`` se ``x`` è minore di ``edge`` e ``1.0`` altrimenti.
...
La porta ``x`` utilizza un tipo di vettore 2D, mentre la porta ``edge`` utilizza un tipo di scalare in virgola mobile.
...
La porta ``x`` utilizza un tipo di vettore 3D, mentre la porta ``edge`` utilizza un tipo di scalare in virgola mobile.
...
Rappresenta la dimensione dell'enumerazione :ref:`OpType<enum_VisualShaderNodeStep_OpType>`.
