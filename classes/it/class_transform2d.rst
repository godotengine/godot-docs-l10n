...
\ **Nota:** In un contesto booleano, un Transform2D sarà valutato come ``false`` se è uguale a :ref:`IDENTITY<class_Transform2D_constant_IDENTITY>`. Altrimenti, un Transform2D sarà sempre valutato come ``true``.
...
\ ``weight`` dovrebbe essere compreso tra ``0.0`` e ``1.0`` (inclusi). I valori fuori da questo intervallo sono consentiti per effettuare un'*estrapolazione*.
