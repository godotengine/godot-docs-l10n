...
Ogg Vorbis è un formato audio lossy (con perdita), con una migliore qualità audio rispetto a :ref:`ResourceImporterMP3<class_ResourceImporterMP3>` a un determinato bitrate.
...
Ogg Vorbis richiede più CPU per la decodifica rispetto a :ref:`ResourceImporterWAV<class_ResourceImporterWAV>`. Se è necessario riprodurre molti suoni allo stesso tempo, si consiglia di utilizzare WAV per quei suoni, soprattutto se destinati a dispositivi di fascia bassa.
...
Ha effetto solo se :ref:`loop<class_ResourceImporterOggVorbis_property_loop>` è ``true``.

Nella finestra di dialogo **Impostazioni d'importazione avanzate** è fornito un editor più comodo per :ref:`loop_offset<class_ResourceImporterOggVorbis_property_loop_offset>`, in quanto consente di visualizzare le modifiche in anteprima senza dover reimportare l'audio.
