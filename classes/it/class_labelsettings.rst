...
Il colore del contorno all'indice ``index``.

\ **Note:** ``index`` è un valore compreso nell'intervallo ``0 .. stacked_outline_count - 1``.
...
La dimensione del contorno all'indice ``index``.
...
Il colore dell'ombra all'indice ``index``.
...
L'offset dell'ombra all'indice ``index``.
...
La dimensione del contorno dell'ombra all'indice ``index``.
