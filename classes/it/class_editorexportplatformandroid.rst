...
- :doc:`Esportazione per Android <../tutorials/export/exporting_for_android>`

- :doc:`Build Gradle per Android <../tutorials/export/android_gradle_build>`

- :doc:`Indice della documentazione dei plugin Android <../tutorials/platform/index>`
...
Se ``true``, i binari per ``arm32`` sono inclusi nel progetto esportato.
...
Se ``true``, i binari per ``x86_32`` sono inclusi nel progetto esportato.
...
Se ``true``, i binari per ``x86_64`` sono inclusi nel progetto esportato.
...
Elenco di argomenti aggiuntivi della riga di comando, separati da spazi, che il progetto esportato riceverà all'avvio.
...
Percorso a un file APK da usare come modello di esportazione personalizzato per le esportazioni di debug. Se lasciato vuoto, viene usato il modello predefinito.

\ **Nota:** Questo viene usato solo se :ref:`gradle_build/use_gradle_build<class_EditorExportPlatformAndroid_property_gradle_build/use_gradle_build>` è disabilitato.
...
Percorso a un file APK da usare come modello di esportazione personalizzato per le esportazioni di rilascio. Se lasciato vuoto, viene usato il modello predefinito.
...
Se ``true``, `Scorri per ignorare <https://developer.android.com/design/ui/wear/guides/components/swipe-to-dismiss>`__ sarà abilitato.
...
\ **Nota:** Questo è ``false`` come predefinito. Per abilitare questo comportamento, è richiesto :ref:`gradle_build/use_gradle_build<class_EditorExportPlatformAndroid_property_gradle_build/use_gradle_build>`.
...
Percorso a un file ZIP contenente la sorgente per il modello di esportazione utilizzato in una build Gradle. Se lasciato vuoto, viene utilizzato il modello predefinito.
...
Formato di esportazione dell'applicazione (\*.apk o \*.aab).
...
Percorso alla cartella di compilazione di Gradle. Se lasciato vuoto, verrà utilizzato ``res://android``.
...
Se ``true``, la build di Gradle è utilizzata invece dell'APK precompilato.
...
Se ``true``, sarà creato il contesto di debug di OpenGL ES (verifiche, convalida e logging aggiuntivi in fase di esecuzione).
...
Può essere sovrascritto con la variabile di ambiente ``GODOT_ANDROID_KEYSTORE_DEBUG_PATH``.

Predefinito su ``EditorSettings.export/android/debug_keystore`` se vuoto.
...
Password per il file keystore di debug.

Può essere sovrascritto con la variabile di ambiente ``GODOT_ANDROID_KEYSTORE_DEBUG_PASSWORD``.

Predefinito su ``EditorSettings.export/android/debug_keystore_pass`` se sia esso sia :ref:`keystore/debug<class_EditorExportPlatformAndroid_property_keystore/debug>` sono vuoti.
...
Nome utente per il file keystore di debug.

Può essere sovrascritto con la variabile di ambiente ``GODOT_ANDROID_KEYSTORE_DEBUG_USER``.

Predefinito su ``EditorSettings.export/android/debug_keystore_user`` se sia esso sia :ref:`keystore/debug<class_EditorExportPlatformAndroid_property_keystore/debug>` sono vuoti.
...
Percorso del file keystore di rilascio.

Può essere sovrascritto con la variabile di ambiente ``GODOT_ANDROID_KEYSTORE_RELEASE_PATH``.
...
Password per il file keystore di rilascio.

Può essere sovrascritta con la variabile di ambiente ``GODOT_ANDROID_KEYSTORE_RELEASE_PASSWORD``.
...
Nome utente per il file keystore di rilascio.

Può essere sovrascritto con la variabile di ambiente ``GODOT_ANDROID_KEYSTORE_RELEASE_USER``.
...
Livello di sfondo del file dell'icona adattiva dell'applicazione. Consulta `Progettare icone adattive <https://developer.android.com/develop/ui/views/launch/icon_design_adaptive#design-adaptive-icons>`__.
...
Livello in primo piano del file dell'icona adattiva dell'applicazione. Consulta `Progettare icone adattive <https://developer.android.com/develop/ui/views/launch/icon_design_adaptive#design-adaptive-icons>`__.
...
Livello monocromatico del file dell'icona adattiva dell'applicazione. Consulta `Progettare icone adattive <https://developer.android.com/develop/ui/views/launch/icon_design_adaptive#design-adaptive-icons>`__.
...
File icona dell'applicazione. Se lasciato vuoto, verrà utilizzato :ref:`ProjectSettings.application/config/icon<class_ProjectSettings_property_application/config/icon>`.
...
Categoria dell'applicazione per il Google Play Store. Definiscilo solo se la tua applicazione rientra bene in una delle categorie. Consulta `android:appCategory <https://developer.android.com/guide/topics/manifest/application-element#appCategory>`__.
...
Se ``true``, l'attività avviata dall'attività principale sarà esclusa dalla lista delle applicazioni utilizzate di recente. Consulta `android:excludeFromRecents <https://developer.android.com/guide/topics/manifest/activity-element#exclude>`__.
...
Se ``true``, quando l'utente disinstalla un'app, sarà visualizzato un suggerimento di conservare i dati dell'app. Consulta `android:hasFragileUserData <https://developer.android.com/guide/topics/manifest/application-element#fragileuserdata>`__.
...
Se ``true``, l'utente sarà in grado di impostare questa applicazione come il lanciatore di sistema nelle preferenze Android.
...
Se ``true``, questa applicazione sarà mostrata nell'interfaccia utente del launcher di Android TV.
...
Se ``true``, questa app sarà visualizzata nella libreria app del dispositivo.

\ **Nota:** Questo è ``true`` per impostazione predefinita.
...
Se ``true``, la firma del pacchetto è abilitata.
...
Identificatore univoco dell'applicazione in un formato DNS inverso. Il formato DNS inverso dovrebbe preferibilmente corrispondere a un nome di dominio che tu controlli, ma ciò non è strettamente necessario. Ad esempio, se possiedi ``example.com``, il nome univoco del tuo pacchetto dovrebbe preferibilmente essere nel formato ``com.example.mygame``. Questo identificatore può contenere solo caratteri alfanumerici minuscoli (``a-z`` e ``0-9``), caratteri di sottolineatura (``_``) e punti (``.``). Ogni componente del formato DNS inverso deve iniziare con una lettera: ad esempio, ``com.example.8game`` non è valido.

Se ``$genname`` è presente nel valore, verrà sostituito dal nome del progetto convertito in minuscolo. Se nel nome del progetto sono presenti caratteri non validi, verranno eliminati. Se tutti i caratteri nel nome del progetto vengono eliminati, ``$genname`` viene sostituito da ``noname``.

\ **Nota:** Modificare il nome del pacchetto farà in modo che il pacchetto venga considerato come un nuovo pacchetto, con i propri percorsi di installazione e dati. Il nuovo pacchetto non potrà essere utilizzato per aggiornare le installazioni esistenti.

\ **Nota:** Quando si pubblica su Google Play, il nome del pacchetto deve essere *globalmente* univoco. Ciò significa che nessun'altra app pubblicata su Google Play deve utilizzare lo stesso nome di pacchetto della tua. Altrimenti, non potrai pubblicare la tua app su Google Play.
...
Consente l'accesso in lettura/scrittura alla tabella "proprietà" nel database di check-in. Consulta `ACCESS_CHECKIN_PROPERTIES <https://developer.android.com/reference/android/Manifest.permission#ACCESS_CHECKIN_PROPERTIES>`__.
...
Consente l'accesso alle informazioni sulla posizione approssimativa. Consulta `ACCESS_COARSE_LOCATION <https://developer.android.com/reference/android/Manifest.permission#ACCESS_COARSE_LOCATION>`__.
...
Consente l'accesso alle informazioni precise sulla posizione. Consulta `ACCESS_FINE_LOCATION <https://developer.android.com/reference/android/Manifest.permission#ACCESS_FINE_LOCATION>`__.
...
Consente l'accesso ai comandi aggiuntivi del fornitore della posizione. Consulta `ACCESS_LOCATION_EXTRA_COMMANDS <https://developer.android.com/reference/android/Manifest.permission#ACCESS_LOCATION_EXTRA_COMMANDS>`__.
...
Consente a un'applicazione di accedere a qualsiasi posizione geografica mantenuta nella raccolta condivisa dell'utente. Consulta `ACCESS_MEDIA_LOCATION <https://developer.android.com/reference/android/Manifest.permission#ACCESS_MEDIA_LOCATION>`__.
...
Consente a un'applicazione di creare fornitori di posizione fittizi per motivi di test.
...
Consente l'accesso alle informazioni sulle reti. Consulta `ACCESS_NETWORK_STATE <https://developer.android.com/reference/android/Manifest.permission#ACCESS_NETWORK_STATE>`__.
...
Consente l'accesso alle informazioni sulle reti Wi-Fi. Consulta `ACCESS_WIFI_STATE <https://developer.android.com/reference/android/Manifest.permission#ACCESS_WIFI_STATE>`__.
...
Consente alle applicazioni di chiamare all'interno dei AccountAuthenticator. Consulta `ACCOUNT_MANAGER <https://developer.android.com/reference/android/Manifest.permission#ACCOUNT_MANAGER>`__.
...
Consente a un'applicazione di aggiungere messaggi vocali al sistema. Consulta `ADD_VOICEMAIL <https://developer.android.com/reference/android/Manifest.permission#ADD_VOICEMAIL>`__.
...
Consente a un'applicazione di fungere da AccountAuthenticator per l'AccountManager.
...
Consente a un'applicazione di raccogliere statistiche sulla batteria. Consulta `BATTERY_STATS <https://developer.android.com/reference/android/Manifest.permission#BATTERY_STATS>`__.
...
Deve essere richiesto da un AccessibilityService, per garantire che solo il sistema possa associarsi ad esso. Consulta `BIND_ACCESSIBILITY_SERVICE <https://developer.android.com/reference/android/Manifest.permission#BIND_ACCESSIBILITY_SERVICE>`__.
...
Consente a un'applicazione di indicare al servizio AppWidget quale applicazione può accedere ai dati di AppWidget. Consulta `BIND_APPWIDGET <https://developer.android.com/reference/android/Manifest.permission#BIND_APPWIDGET>`__.
...
Deve essere richiesto dal ricevitore dell'amministrazione del dispositivo, per garantire che solo il sistema possa interagire con esso. Consulta `BIND_DEVICE_ADMIN <https://developer.android.com/reference/android/Manifest.permission#BIND_DEVICE_ADMIN>`__.
...
Deve essere richiesto da un InputMethodService, per garantire che solo il sistema possa associarsi a esso. Consulta `BIND_INPUT_METHOD <https://developer.android.com/reference/android/Manifest.permission#BIND_INPUT_METHOD>`__.
...
Deve essere richiesto da un HostApduService o OffHostApduService per garantire che solo il sistema possa associarsi a esso. Consulta `BIND_NFC_SERVICE <https://developer.android.com/reference/android/Manifest.permission#BIND_NFC_SERVICE>`__.
...
Deve essere richiesto da un NotificationListenerService, per garantire che solo il sistema possa associarsi a esso. Consulta `BIND_NOTIFICATION_LISTENER_SERVICE <https://developer.android.com/reference/android/Manifest.permission#BIND_NOTIFICATION_LISTENER_SERVICE>`__.
...
Deve essere richiesto da un PrintService, per garantire che solo il sistema possa associarsi a esso. Consulta `BIND_PRINT_SERVICE <https://developer.android.com/reference/android/Manifest.permission#BIND_PRINT_SERVICE>`__.
...
Deve essere richiesto da un RemoteViewsService, per garantire che solo il sistema possa associarsi a esso. Consulta `BIND_REMOTEVIEWS <https://developer.android.com/reference/android/Manifest.permission#BIND_REMOTEVIEWS>`__.
...
Deve essere richiesto da un TextService (ad esempio SpellCheckerService) per garantire che solo il sistema possa associarsi a esso. Consulta `BIND_TEXT_SERVICE <https://developer.android.com/reference/android/Manifest.permission#BIND_TEXT_SERVICE>`__.
...
Deve essere richiesto da un VpnService, per garantire che solo il sistema possa associarsi a esso. Consulta `BIND_VPN_SERVICE <https://developer.android.com/reference/android/Manifest.permission#BIND_VPN_SERVICE>`__.
...
Deve essere richiesto da un WallpaperService, per garantire che solo il sistema possa associarsi a esso. Consulta `BIND_WALLPAPER <https://developer.android.com/reference/android/Manifest.permission#BIND_WALLPAPER>`__.
...
Consente alle applicazioni di connettersi ai dispositivi Bluetooth associati. Consulta `BLUETOOTH <https://developer.android.com/reference/android/Manifest.permission#BLUETOOTH>`__.
...
Consente alle applicazioni di rilevare e associare dispositivi Bluetooth. Consulta `BLUETOOTH_ADMIN <https://developer.android.com/reference/android/Manifest.permission#BLUETOOTH_ADMIN>`__.
...
Consente alle applicazioni di associare dispositivi Bluetooth senza interazione dell'utente e di consentire o impedire l'accesso alla rubrica o ai messaggi. Consulta `BLUETOOTH_PRIVILEGED <https://developer.android.com/reference/android/Manifest.permission#BLUETOOTH_PRIVILEGED>`__.
...
Necessario per poter disattivare il dispositivo (molto pericoloso!).
...
Consente a un'applicazione di trasmettere una notifica che un pacchetto dell'applicazione è stato rimosso. Consulta `BROADCAST_PACKAGE_REMOVED <https://developer.android.com/reference/android/Manifest.permission#BROADCAST_PACKAGE_REMOVED>`__.
...
Consente a un'applicazione di trasmettere una notifica di ricezione SMS. Consulta `BROADCAST_SMS <https://developer.android.com/reference/android/Manifest.permission#BROADCAST_SMS>`__.
...
Consente a un'applicazione di trasmettere intenti sticky. Consulta `BROADCAST_STICKY <https://developer.android.com/reference/android/Manifest.permission#BROADCAST_STICKY>`__.
...
Consente a un'applicazione di trasmettere una notifica di ricezione WAP PUSH. Consulta `BROADCAST_WAP_PUSH <https://developer.android.com/reference/android/Manifest.permission#BROADCAST_WAP_PUSH>`__.
...
Consente a un'applicazione di avviare una chiamata telefonica senza passare attraverso l'interfaccia utente del dialer. Consulta `CALL_PHONE <https://developer.android.com/reference/android/Manifest.permission#CALL_PHONE>`__.
...
Consente a un'applicazione di chiamare qualsiasi numero di telefono, compresi i numeri di emergenza, senza passare attraverso l'interfaccia utente del dialer. Consulta `CALL_PRIVILEGED <https://developer.android.com/reference/android/Manifest.permission#CALL_PRIVILEGED>`__.
...
Necessario per poter accedere al dispositivo della fotocamera. Consulta `CAMERA <https://developer.android.com/reference/android/Manifest.permission#CAMERA>`__.
...
Consente a un'applicazione di catturare un'uscita audio. Consulta `CAPTURE_AUDIO_OUTPUT <https://developer.android.com/reference/android/Manifest.permission#CAPTURE_AUDIO_OUTPUT>`__.
...
Consente a un'applicazione di acquisire un'uscita video sicura.
...
Consente a un'applicazione di acquisire un'uscita video.
...
Consente a un'applicazione di modificare se un componente dell'applicazione (diverso dal proprio) è abilitato o meno. Consulta `CHANGE_COMPONENT_ENABLED_STATE <https://developer.android.com/reference/android/Manifest.permission#CHANGE_COMPONENT_ENABLED_STATE>`__.
...
Consente a un'applicazione di modificare la configurazione attuale, ad esempio le impostazioni locali. Consulta `CHANGE_CONFIGURATION <https://developer.android.com/reference/android/Manifest.permission#CHANGE_CONFIGURATION>`__.
...
Consente alle applicazioni di modificare lo stato di connettività della rete. Consulta `CHANGE_NETWORK_STATE <https://developer.android.com/reference/android/Manifest.permission#CHANGE_NETWORK_STATE>`__.
...
Consente alle applicazioni di accedere alla modalità Wi-Fi Multicast. Consulta `CHANGE_WIFI_MULTICAST_STATE <https://developer.android.com/reference/android/Manifest.permission#CHANGE_WIFI_MULTICAST_STATE>`__.
...
Consente alle applicazioni di modificare lo stato della connettività Wi-Fi. Consulta `CHANGE_WIFI_STATE <https://developer.android.com/reference/android/Manifest.permission#CHANGE_WIFI_STATE>`__.
...
Consente a un'applicazione di cancellare le cache di tutte le applicazioni installate sul dispositivo. Consulta `CLEAR_APP_CACHE <https://developer.android.com/reference/android/Manifest.permission#CLEAR_APP_CACHE>`__.
...
Consente di abilitare/disabilitare le notifiche di aggiornamento della posizione dalla radio. Consulta `CONTROL_LOCATION_UPDATES <https://developer.android.com/reference/android/Manifest.permission#CONTROL_LOCATION_UPDATES>`__.
...
Array di stringhe di autorizzazione personalizzate.
...
Consente a un'applicazione di eliminare pacchetti. Consulta `DELETE_PACKAGES <https://developer.android.com/reference/android/Manifest.permission#DELETE_PACKAGES>`__.
...
Consente l'accesso di basso livello alla gestione dell'alimentazione.
...
Consente alle applicazioni di eseguire operazioni di lettura e di scrittura per diagnosticare le risorse. Consulta `DIAGNOSTIC <https://developer.android.com/reference/android/Manifest.permission#DIAGNOSTIC>`__.
...
Consente alle applicazioni di disattivare la protezione della tastiera se non è sicura. Consulta `DISABLE_KEYGUARD <https://developer.android.com/reference/android/Manifest.permission#DISABLE_KEYGUARD>`__.
...
Consente a un'applicazione di recuperare informazioni di dump di stato dai servizi di sistema. Consulta `DUMP <https://developer.android.com/reference/android/Manifest.permission#DUMP>`__.
...
Consente a un'applicazione di espandere o comprimere la barra di stato. Consulta `EXPAND_STATUS_BAR <https://developer.android.com/reference/android/Manifest.permission#EXPAND_STATUS_BAR>`__.
...
Esegue come applicazione di test del produttore, in esecuzione come utente root. Consulta `FACTORY_TEST <https://developer.android.com/reference/android/Manifest.permission#FACTORY_TEST>`__.
...
Consente a un'applicazione di forzare un'operazione BACK (Indietro) su qualsiasi attività sia in cima.
...
Consente l'accesso alla lista degli account nel servizio Account. Consulta `GET_ACCOUNTS <https://developer.android.com/reference/android/Manifest.permission#GET_ACCOUNTS>`__.
...
Consente a un'applicazione di scoprire lo spazio utilizzato da qualsiasi pacchetto. Consulta `GET_PACKAGE_SIZE <https://developer.android.com/reference/android/Manifest.permission#GET_PACKAGE_SIZE>`__.
...
**Deprecato:** Deprecated in API level 21.
...
Consente a un'applicazione di recuperare informazioni private sull'attività principale attuale.
...
Utilizzato sui fornitori di contenuti per consentire al sistema di ricerca globale di accedere ai loro dati. Consulta `GLOBAL_SEARCH <https://developer.android.com/reference/android/Manifest.permission#GLOBAL_SEARCH>`__.
...
Consente a un'applicazione di inserire eventi utente (tasti, tocco, trackball) nel flusso di eventi e di inviarli a QUALSIASI finestra.
...
Consente a un'applicazione di installare un fornitore di posizione nel Location Manager. Consulta `INSTALL_LOCATION_PROVIDER <https://developer.android.com/reference/android/Manifest.permission#INSTALL_LOCATION_PROVIDER>`__.
...
Consente a un'applicazione di installare pacchetti. Consulta `INSTALL_PACKAGES <https://developer.android.com/reference/android/Manifest.permission#INSTALL_PACKAGES>`__.
...
Consente a un'applicazione di installare un collegamento in Launcher. Consulta `INSTALL_SHORTCUT <https://developer.android.com/reference/android/Manifest.permission#INSTALL_SHORTCUT>`__.
...
Consente a un'applicazione di aprire finestre usate da alcune parti dell'interfaccia utente del sistema.
...
Consente alle applicazioni di aprire socket di rete. Consulta `INTERNET <https://developer.android.com/reference/android/Manifest.permission#INTERNET>`__.
...
Consente a un'applicazione di chiamare ActivityManager.killBackgroundProcesses(String). Consulta `KILL_BACKGROUND_PROCESSES <https://developer.android.com/reference/android/Manifest.permission#KILL_BACKGROUND_PROCESSES>`__.
...
Consente a un'applicazione di utilizzare le funzionalità di posizione nell'hardware, come l'API di geofencing. Consulta `LOCATION_HARDWARE <https://developer.android.com/reference/android/Manifest.permission#LOCATION_HARDWARE>`__.
...
Consente a un'applicazione di gestire la lista degli account nell'AccountManager.
...
Consente a un'applicazione di gestire (creare, distruggere, ordinare in Z) i token dell'applicazione nel gestore delle finestre.
...
Consente a un'applicazione di gestire l'accesso ai documenti, solitamente come parte di un selettore di documenti. Consulta `MANAGE_DOCUMENTS <https://developer.android.com/reference/android/Manifest.permission#MANAGE_DOCUMENTS>`__.
...
Consente a un'applicazione un ampio accesso all'archiviazione esterna in un archivio delimitato. Consulta `MANAGE_EXTERNAL_STORAGE <https://developer.android.com/reference/android/Manifest.permission#MANAGE_EXTERNAL_STORAGE>`__.
...
Consulta `MASTER_CLEAR <https://developer.android.com/reference/android/Manifest.permission#MASTER_CLEAR>`__.
...
Consente a un'applicazione di sapere quale contenuto è in riproduzione e di controllarne la riproduzione. Consulta `MEDIA_CONTENT_CONTROL <https://developer.android.com/reference/android/Manifest.permission#MEDIA_CONTENT_CONTROL>`__.
...
Consente a un'applicazione di modificare le impostazioni audio globali. Consulta `MODIFY_AUDIO_SETTINGS <https://developer.android.com/reference/android/Manifest.permission#MODIFY_AUDIO_SETTINGS>`__.
...
Consente la modifica dello stato della telefonia: accensione, mmi, ecc. Non include l'effettuazione di chiamate. Consulta `MODIFY_PHONE_STATE <https://developer.android.com/reference/android/Manifest.permission#MODIFY_PHONE_STATE>`__.
...
Consente la formattazione dei file system per archiviazioni rimovibili. Consulta `MOUNT_FORMAT_FILESYSTEMS <https://developer.android.com/reference/android/Manifest.permission#MOUNT_FORMAT_FILESYSTEMS>`__.
...
Consente il montaggio e lo smontaggio dei file system per archiviazioni rimovibili. Consulta `MOUNT_UNMOUNT_FILESYSTEMS <https://developer.android.com/reference/android/Manifest.permission#MOUNT_UNMOUNT_FILESYSTEMS>`__.
...
Consente alle applicazioni di eseguire operazioni di I/O tramite NFC. Consulta `NFC <https://developer.android.com/reference/android/Manifest.permission#NFC>`__.
...
**Deprecato:** Deprecated in API level 15.

Consente a un'applicazione di rendere persistenti le proprie attività.
...
Consente a un'applicazione di pubblicare notifiche. Aggiunto nel livello API 33. Consulta `Permesso di notifiche durante l'esecuzione <https://developer.android.com/develop/ui/views/notifications/notification-permission>`__.
...
**Deprecato:** Deprecated in API level 29.

Consente a un'applicazione di vedere il numero composto durante una chiamata in uscita con l'opzione di reindirizzare la chiamata a un numero diverso o di interromperla del tutto. Consulta `PROCESS_OUTGOING_CALLS <https://developer.android.com/reference/android/Manifest.permission#PROCESS_OUTGOING_CALLS>`__.
...
Consente a un'applicazione di leggere i dati del calendario dell'utente. Consulta `READ_CALENDAR <https://developer.android.com/reference/android/Manifest.permission#READ_CALENDAR>`__.
...
Consente a un'applicazione di leggere il registro delle chiamate dell'utente. Consulta `READ_CALL_LOG <https://developer.android.com/reference/android/Manifest.permission#READ_CALL_LOG>`__.
...
Consente a un'applicazione di leggere i dati dei contatti dell'utente. Consulta `READ_CONTACTS <https://developer.android.com/reference/android/Manifest.permission#READ_CONTACTS>`__.
...
**Deprecato:** Deprecated in API level 33.

Consente a un'applicazione di leggere da un archivio esterno. Consulta `READ_EXTERNAL_STORAGE <https://developer.android.com/reference/android/Manifest.permission#READ_EXTERNAL_STORAGE>`__.
...
Consente a un'applicazione di fare screenshot e, più generalmente, di accedere ai dati del frame buffer.
...
**Deprecato:** Deprecated in API level 16.
...
Consente a un'applicazione di leggere i file registro di basso livello del sistema . Consulta `READ_LOGS <https://developer.android.com/reference/android/Manifest.permission#READ_LOGS>`__.
...
Consente a un'applicazione di leggere file audio da una memoria esterna. Consulta `READ_MEDIA_AUDIO <https://developer.android.com/reference/android/Manifest.permission#READ_MEDIA_AUDIO>`__.
...
Consente a un'applicazione di leggere file immagine da un archivio esterno. Consulta `READ_MEDIA_IMAGES <https://developer.android.com/reference/android/Manifest.permission#READ_MEDIA_IMAGES>`__.
...
Consente a un'applicazione di leggere file video da un archivio esterno. Consulta `READ_MEDIA_VIDEO <https://developer.android.com/reference/android/Manifest.permission#READ_MEDIA_VIDEO>`__.
...
Consente l'accesso in sola lettura allo stato del telefono. Consulta `READ_PHONE_STATE <https://developer.android.com/reference/android/Manifest.permission#READ_PHONE_STATE>`__.
...
Consente a un'applicazione di leggere i messaggi SMS. Consulta `READ_SMS <https://developer.android.com/reference/android/Manifest.permission#READ_SMS>`__.
...
Consente a un'applicazione di leggere dal flusso sociale dell'utente.
...
Consente alle applicazioni di leggere le impostazioni di sincronizzazione. Consulta `READ_SYNC_SETTINGS <https://developer.android.com/reference/android/Manifest.permission#READ_SYNC_SETTINGS>`__.
...
Consente alle applicazioni di leggere le statistiche di sincronizzazione. Consulta `READ_SYNC_STATS <https://developer.android.com/reference/android/Manifest.permission#READ_SYNC_STATS>`__.
...
Richiesto per poter riavviare il dispositivo. Consulta `REBOOT <https://developer.android.com/reference/android/Manifest.permission#REBOOT>`__.
...
Consente a un'applicazione di ricevere Intent.ACTION_BOOT_COMPLETED che viene trasmesso dopo che il sistema ha terminato l'avvio. Consulta `RECEIVE_BOOT_COMPLETED <https://developer.android.com/reference/android/Manifest.permission#RECEIVE_BOOT_COMPLETED>`__.
...
Consente a un'applicazione di monitorare i messaggi MMS in arrivo. Consulta `RECEIVE_MMS <https://developer.android.com/reference/android/Manifest.permission#RECEIVE_MMS>`__.
...
Consente a un'applicazione di ricevere i messaggi SMS. Consulta `RECEIVE_MMS <https://developer.android.com/reference/android/Manifest.permission#RECEIVE_MMS>`__.
...
Consente a un'applicazione di ricevere messaggi push WAP. Consulta `RECEIVE_WAP_PUSH <https://developer.android.com/reference/android/Manifest.permission#RECEIVE_WAP_PUSH>`__.
...
Consente a un'applicazione di registrare audio. Consulta `RECORD_AUDIO <https://developer.android.com/reference/android/Manifest.permission#RECORD_AUDIO>`__.
...
Consente a un'applicazione di cambiare l'ordine Z delle attività. Consulta `REORDER_TASKS <https://developer.android.com/reference/android/Manifest.permission#REORDER_TASKS>`__.
...
Consente a un'applicazione (telefono) di inviare una richiesta ad altre applicazioni per gestire l'azione di risposta tramite messaggio durante le chiamate in arrivo. Consulta `SEND_RESPOND_VIA_MESSAGE <https://developer.android.com/reference/android/Manifest.permission#SEND_RESPOND_VIA_MESSAGE>`__.
...
Consente a un'applicazione di inviare i messaggi SMS. Consulta `RECEIVE_MMS <https://developer.android.com/reference/android/Manifest.permission#RECEIVE_MMS>`__.
...
Consente a un'applicazione di osservare e controllare il modo in cui le attività vengono avviate a globalmente nel sistema.
...
Consente a un'applicazione di trasmettere un Intent per impostare una sveglia per l'utente. Consulta `SET_ALARM <https://developer.android.com/reference/android/Manifest.permission#SET_ALARM>`__.
...
Consente a un'applicazione di controllare se le attività vengono immediatamente terminate quando vengono messe nello sfondo. Consulta `SET_ALWAYS_FINISH <https://developer.android.com/reference/android/Manifest.permission#SET_ALWAYS_FINISH>`__.
...
Consente di modificare il fattore globale di scala delle animazioni. Consulta `SET_ANIMATION_SCALE <https://developer.android.com/reference/android/Manifest.permission#SET_ANIMATION_SCALE>`__.
...
Configura un'applicazione per il debug. Consulta `SET_DEBUG_APP <https://developer.android.com/reference/android/Manifest.permission#SET_DEBUG_APP>`__.
...
Consente l'accesso di basso livello all'impostazione della velocità del puntatore.
...
Consente alle applicazioni di impostare direttamente l'orario del sistema. Consulta `SET_TIME <https://developer.android.com/reference/android/Manifest.permission#SET_TIME>`__.
...
Consente alle applicazioni di impostare direttamente il fuso orario del sistema. Consulta `SET_TIME_ZONE <https://developer.android.com/reference/android/Manifest.permission#SET_TIME_ZONE>`__.
...
Consente alle applicazioni di impostare lo sfondo. Consulta `SET_WALLPAPER <https://developer.android.com/reference/android/Manifest.permission#SET_WALLPAPER>`__.
...
Consente alle applicazioni di impostare i suggerimenti per lo sfondo. Consulta `SET_WALLPAPER_HINTS <https://developer.android.com/reference/android/Manifest.permission#SET_WALLPAPER_HINTS>`__.
...
Consenti a un'applicazione di richiedere che un segnale sia inviato a tutti i processi persistenti. Consulta `SIGNAL_PERSISTENT_PROCESSES <https://developer.android.com/reference/android/Manifest.permission#SIGNAL_PERSISTENT_PROCESSES>`__.
...
Consente a un'applicazione di aprire, chiudere o disabilitare la barra di stato e le sue icone. Consulta `STATUS_BAR <https://developer.android.com/reference/android/Manifest.permission#STATUS_BAR>`__.
...
Consente a un'applicazione di consentire l'accesso al ContentProvider dei feed sottoscritti.
...
Consente a un'applicazione di creare finestre tramite il tipo WindowManager.LayoutParams.TYPE_APPLICATION_OVERLAY, mostrato sopra tutte le altre applicazioni. Consulta `SYSTEM_ALERT_WINDOW <https://developer.android.com/reference/android/Manifest.permission#SYSTEM_ALERT_WINDOW>`__.
...
Consente di utilizzare il trasmettitore IR del dispositivo, se disponibile. Consulta `TRANSMIT_IR <https://developer.android.com/reference/android/Manifest.permission#TRANSMIT_IR>`__.
...
Consente a un'applicazione di aggiornare le statistiche del dispositivo. Consulta `UPDATE_DEVICE_STATS <https://developer.android.com/reference/android/Manifest.permission#UPDATE_DEVICE_STATS>`__.
...
Consente a un'applicazione di richiedere token di autenticazione dall'AccountManager.
...
Consente a un'applicazione di utilizzare il servizio SIP. Consulta `USE_SIP <https://developer.android.com/reference/android/Manifest.permission#USE_SIP>`__.
...
Consente l'accesso al vibratore. Consulta `VIBRATE <https://developer.android.com/reference/android/Manifest.permission#VIBRATE>`__.
...
Consente di utilizzare i WakeLock del PowerManager per impedire al processore di sospendersi o allo schermo di oscurarsi. Consulta `WAKE_LOCK <https://developer.android.com/reference/android/Manifest.permission#WAKE_LOCK>`__.
...
Consente alle applicazioni di scrivere le impostazioni apn e di leggere i campi sensibili di impostazioni apn esistenti, come utente e password. Consulta `WRITE_APN_SETTINGS <https://developer.android.com/reference/android/Manifest.permission#WRITE_APN_SETTINGS>`__.
...
Consente a un'applicazione di scrivere i dati del calendario dell'utente. Consulta `WRITE_CALENDAR <https://developer.android.com/reference/android/Manifest.permission#WRITE_CALENDAR>`__.
...
Consente a un'applicazione di scrivere (ma non di leggere) i dati del registro delle chiamate dell'utente. Consulta `WRITE_CALL_LOG <https://developer.android.com/reference/android/Manifest.permission#WRITE_CALL_LOG>`__.
...
Consente a un'applicazione di scrivere i dati dei contatti dell'utente. Consulta `WRITE_CONTACTS <https://developer.android.com/reference/android/Manifest.permission#WRITE_CONTACTS>`__.
...
Consente a un'applicazione di scrivere su una memoria esterna. Consulta `WRITE_EXTERNAL_STORAGE <https://developer.android.com/reference/android/Manifest.permission#WRITE_EXTERNAL_STORAGE>`__.
...
Consente a un'applicazione di modificare la mappa dei servizi Google. Consulta `WRITE_GSERVICES <https://developer.android.com/reference/android/Manifest.permission#WRITE_GSERVICES>`__.
...
Consente a un'applicazione di scrivere (ma non di leggere) la cronologia di navigazione e i segnalibri dell'utente.
...
Consente a un'applicazione di scrivere (ma non di leggere) i dati del profilo personale dell'utente.
...
Consente a un'applicazione di leggere o scrivere le impostazioni sicure del sistema. Consulta `WRITE_SECURE_SETTINGS <https://developer.android.com/reference/android/Manifest.permission#WRITE_SECURE_SETTINGS>`__.
...
Consente a un'applicazione di leggere o scrivere le impostazioni del sistema. Consulta `WRITE_SETTINGS <https://developer.android.com/reference/android/Manifest.permission#WRITE_SETTINGS>`__.
...
Consente a un'applicazione di scrivere (ma non di leggere) i dati del flusso social dell'utente.
...
Consente alle applicazioni di scrivere le impostazioni di sincronizzazione. Consulta `WRITE_SYNC_SETTINGS <https://developer.android.com/reference/android/Manifest.permission#WRITE_SYNC_SETTINGS>`__.
...
Indica se un'applicazione supporta i fattori di forma più grandi dello schermo.
...
Indica se un'applicazione supporta i fattori di forma "normali" dello schermo.
...
Indica se l'applicazione supporta fattori forma di schermi più piccoli.
...
Indica se l'applicazione supporta fattori forma di schermi di grandi dimensioni.
...
Se ``true``, consente all'applicazione di partecipare all'infrastruttura di backup e ripristino.
...
Versione dell'applicazione leggibile dalla macchina. Deve essere incrementata per ogni nuova versione inviata al Play Store.
...
Versione dell'applicazione visibile all'utente. Ricade su :ref:`ProjectSettings.application/config/version<class_ProjectSettings_property_application/config/version>` se lasciato vuoto.
...
Modalità di realtà estesa (XR) per questa applicazione.
