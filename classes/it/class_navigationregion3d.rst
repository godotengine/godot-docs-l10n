...
Una regione 3D attraversabile che gli :ref:`NavigationAgent3D<class_NavigationAgent3D>` possono utilizzare per la ricerca del percorso.
...
Una regione 2D attraversabile basata su un :ref:`NavigationMesh<class_NavigationMesh>` che i :ref:`NavigationAgent3D<class_NavigationAgent3D>` possono utilizzare per la ricerca del percorso.

Due regioni possono essere collegate tra loro se condividono un bordo simile. Puoi impostare la distanza minima tra due vertici richiesta per collegare due bordi tramite :ref:`NavigationServer3D.map_set_edge_connection_margin()<class_NavigationServer3D_method_map_set_edge_connection_margin>`.
...
Il costo per entrare in questa regione da un'altra regione può essere controllato con la proprietà :ref:`enter_cost<class_NavigationRegion3D_property_enter_cost>`.
...
Il costo per percorrere distanze all'interno di questa regione può essere controllato con il moltiplicatore :ref:`travel_cost<class_NavigationRegion3D_property_travel_cost>`.

\ **Nota:** Questo nodo memorizza nella cache le modifiche apportate alle sue proprietà, quindi se si apportano modifiche al :ref:`RID<class_RID>` della regione sottostante nel :ref:`NavigationServer3D<class_NavigationServer3D>`, queste non saranno riflesse nelle proprietà di questo nodo.
...
Notifica quando il :ref:`NavigationMesh<class_NavigationMesh>` è cambiato.
...
Quando la ricerca del percorso entra nella mesh di navigazione di questa regione dalla mesh di navigazione di un'altra regione, il valore di :ref:`enter_cost<class_NavigationRegion3D_property_enter_cost>` viene aggiunto alla distanza del percorso per determinare il percorso più breve.
...
Un campo di bit che determina tutti gli strati di navigazione a cui appartiene la regione. Questi strati di navigazione possono essere controllati quando si richiede un percorso con :ref:`NavigationServer3D.map_get_path()<class_NavigationServer3D_method_map_get_path>`.
...
La risorsa :ref:`NavigationMesh<class_NavigationMesh>` da utilizzare.
...
Quando la ricerca del percorso si muove all'interno della mesh di navigazione di questa regione, le distanze percorse vengono moltiplicate per il valore di :ref:`travel_cost<class_NavigationRegion3D_property_travel_cost>` per determinare il percorso più breve.
...
Restituisce se lo strato specificato della maschera di bit :ref:`navigation_layers<class_NavigationRegion3D_property_navigation_layers>` è abilitato o meno, fornito un ``layer_number`` tra 1 e 32.
...
**Deprecato:** Use :ref:`get_rid()<class_NavigationRegion3D_method_get_rid>` instead.

Restituisce il :ref:`RID<class_RID>` di questa regione sul :ref:`NavigationServer3D<class_NavigationServer3D>`.
...
Restituisce il :ref:`RID<class_RID>` di questa regione sul :ref:`NavigationServer3D<class_NavigationServer3D>`. In combinazione con :ref:`NavigationServer3D.map_get_closest_point_owner()<class_NavigationServer3D_method_map_get_closest_point_owner>`, questo metodo può essere utilizzato per identificare il **NavigationRegion3D** più vicino a un punto sulla mappa unita di navigazione.
...
Restituisce ``true`` quando il :ref:`NavigationMesh<class_NavigationMesh>` è in fase di preparazione su un thread in background.
...
In base al ``value``, abilita o disabilita lo strato specificato nella maschera di bit :ref:`navigation_layers<class_NavigationRegion3D_property_navigation_layers>`, fornito un ``layer_number`` compreso tra 1 e 32.
