...
Массив, специально разработанный для хранения 32-битных целых чисел. Плотно упаковывает данные, поэтому экономит память при больших размерах массива.

\ **Примечание:** Этот тип хранит 32-битные целые числа со знаком, что означает, что он может принимать значения в интервале ``[-2^31, 2^31 - 1]``, т.е. ``[-2147483648, 2147483647]``. Превышение этих границ приведет к циклическому переполнению. Для сравнения, :ref:`int<class_int>` использует 64-битные целые числа со знаком, которые могут хранить гораздо большие значения. Если вам нужно плотно упаковывать 64-битные целые числа, см. :ref:`PackedInt64Array<class_PackedInt64Array>`.
...
Возвращает 32-битное целое число по заданному ``index`` в массиве. Если ``index`` выходит за пределы массива или является отрицательным, этот метод завершается с ошибкой и возвращает ``0``.
