...
\ **Примечание:** В логическом контексте кватернион будет оцениваться как ``false``, если он равен :ref:`IDENTITY<class_Quaternion_constant_IDENTITY>`. В противном случае кватернион всегда будет оцениваться как ``true``.
...
Создает новый **Quaternion** из заданного :ref:`Vector3<class_Vector3>`, `углов Эйлера <https://en.wikipedia.org/wiki/Euler_angles>`__ в радианах. В Godot углы Эйлера всегда используют внутренний порядок. Этот метод всегда использует внутреннюю конвенцию YXZ (:ref:`@GlobalScope.EULER_ORDER_YXZ<class_@GlobalScope_constant_EULER_ORDER_YXZ>`).
