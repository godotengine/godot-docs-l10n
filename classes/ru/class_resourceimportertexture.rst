...
Если ``true``, масштабирует импортированное изображение в соответствии с :ref:`EditorSettings.interface/editor/appearance/custom_display_scale<class_EditorSettings_property_interface/editor/appearance/custom_display_scale>`. Эту опцию следует включить для значков плагинов редактора и значков пользовательских классов, но в противном случае её следует оставить отключенной.
...
Альтернативой исправлению затемненных границ с помощью :ref:`process/fix_alpha_border<class_ResourceImporterTexture_property_process/fix_alpha_border>` является использование предварительно умноженного альфа-канала. При включении этой опции текстура будет преобразована в этот формат. Для корректного отображения текстуры с предварительно умноженным альфа-каналом требуются определенные материалы:
