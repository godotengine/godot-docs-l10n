...
用于管理 :ref:`NavigationServer3D<class_NavigationServer3D>` 实现的单例。
...
**NavigationServer3DManager** 是用于注册 :ref:`NavigationServer3D<class_NavigationServer3D>` 实现并设置默认实现的 API。
...
通过传递 ``name`` 和一个返回 :ref:`NavigationServer3D<class_NavigationServer3D>` 对象的 :ref:`Callable<class_Callable>`\ ，注册一个 :ref:`NavigationServer3D<class_NavigationServer3D>` 实现。
...
如果 ``priority`` 的值高于当前默认实现的优先级，则将默认的 :ref:`NavigationServer3D<class_NavigationServer3D>` 实现设置为由 ``name`` 指定的实现。
