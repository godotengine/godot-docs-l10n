...
\ ``display_name`` 是向用户显示的描述。\ ``openxr_path`` 是正被注册的交互配置文件路径。\ ``openxr_extension_names`` 可以选择将该配置文件限制为对给定扩展启用/可用。如果该扩展不可用，则动作映射中使用的配置文件和所有相关条目都会被过滤掉。
...
允许将旧的输入/输出路径重命名为新路径，以便加载和处理旧动作映射。
...
允许将旧的交互配置文件路径重命名为新路径，以便加载和处理旧动作映射。
...
\ ``display_name`` 是向用户显示的名称。\ ``openxr_path`` 是正被注册的顶级路径。\ ``openxr_extension_names`` 是可选的，可确保仅在指定的扩展可用/启用时才使用顶级路径。
