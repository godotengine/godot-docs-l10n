...
如果为 ``true``\ ，则该修改器的 :ref:`_execute<class_SkeletonModification2D_method__execute>` 函数将被该 :ref:`SkeletonModificationStack2D<class_SkeletonModificationStack2D>` 调用。
...
返回这个修改器是否会在 Godot 编辑器中调用 :ref:`_draw_editor_gizmo<class_SkeletonModification2D_method__draw_editor_gizmo>` 绘制针对修改器的小工具。
...
设置这个修改器是否会在 Godot 编辑器中调用 :ref:`_draw_editor_gizmo<class_SkeletonModification2D_method__draw_editor_gizmo>` 绘制针对修改器的小工具。
