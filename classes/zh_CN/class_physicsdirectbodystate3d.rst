...
提供对 :ref:`PhysicsServer3D<class_PhysicsServer3D>` 中物理体的直接访问，从而安全地更改物理属性。该对象会在 :ref:`RigidBody3D<class_RigidBody3D>` 的直接状态回调中传递，目的是为了改变物体的直接状态。见 :ref:`RigidBody3D._integrate_forces<class_RigidBody3D_method__integrate_forces>`\ 。
