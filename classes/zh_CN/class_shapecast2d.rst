...
形状的目标点，相对于该节点的 :ref:`Node2D.position<class_Node2D_property_position>`\ 。
...
\ **注意：**\ 不将 :ref:`enabled<class_ShapeCast2D_property_enabled>` 设置为 ``true`` 也可以生效。
...
返回从投射原点到 :ref:`target_position<class_ShapeCast2D_property_target_position>` 之间的分数，形状在不触发碰撞的情况下能够移动多远，是一个 ``0.0`` 到 ``1.0`` 之间的值。
...
返回从投射原点到 :ref:`target_position<class_ShapeCast2D_property_target_position>` 之间的分数，形状移动多远才能够触发碰撞，是一个 ``0.0`` 到 ``1.0`` 之间的值。

在理想条件下与 :ref:`get_closest_collision_safe_fraction()<class_ShapeCast2D_method_get_closest_collision_safe_fraction>` 相同，但是形状投射是分步骤计算的，因此精确的碰撞点可能发生在两个计算位置之间。
