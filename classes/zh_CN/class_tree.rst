...
如果为 ``true``\ ，则在拖放过程中，当鼠标悬停在树项目上时，树项目将会展开。该操作的延迟时间由 :ref:`dragging_unfold_wait_msec<class_Tree_theme_constant_dragging_unfold_wait_msec>` 决定。
...
如果为 ``true``\ ，则滚动提示纹理将平铺而不是拉伸。见 :ref:`scroll_hint_mode<class_Tree_property_scroll_hint_mode>`\ 。
...
将给定 ``column`` 的标题的语言代码设置为 ``language``\ 。用于断行和文本塑形算法。如果 ``language`` 为空，则使用当前区域设置。
...
用于调制 :ref:`scroll_hint<class_Tree_theme_icon_scroll_hint>` 纹理的 :ref:`Color<class_Color>`\ 。
...
在 :ref:`TreeItem.CELL_MODE_CHECK<class_TreeItem_constant_CELL_MODE_CHECK>` 模式的单元格中，复选框和文本之间的水平间距。
...
内容仍可滚动时将显示的指示器。见 :ref:`scroll_hint_mode<class_Tree_property_scroll_hint_mode>`\ 。
...
项目处于悬停且选中状态时的 :ref:`StyleBox<class_StyleBox>`\ ，在 **Tree** 未获得焦点时使用。
...
项目处于悬停且选中状态时的 :ref:`StyleBox<class_StyleBox>`\ ，在 **Tree** 获得焦点时使用。
