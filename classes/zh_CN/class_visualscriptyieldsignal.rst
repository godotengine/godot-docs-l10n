...
- **CALL_MODE_SELF** = **0** --- 将使用来自此 :ref:`Object<class_Object>` 的信号。

- **CALL_MODE_NODE_PATH** = **1** --- 将使用来自场景树中给定 :ref:`Node<class_Node>` 的信号。
...
:ref:`call_mode<class_VisualScriptYieldSignal_property_call_mode>` 设置为 :ref:`CALL_MODE_INSTANCE<class_VisualScriptYieldSignal_constant_CALL_MODE_INSTANCE>` 时要使用的基本类型。
...
``call_mode`` 确定等待信号发射的目标对象。有关选项，请参阅 :ref:`CallMode<enum_VisualScriptYieldSignal_CallMode>` 。
...
:ref:`call_mode<class_VisualScriptYieldSignal_property_call_mode>` 设置为 :ref:`CALL_MODE_NODE_PATH<class_VisualScriptYieldSignal_constant_CALL_MODE_NODE_PATH>` 时使用的节点路径。
