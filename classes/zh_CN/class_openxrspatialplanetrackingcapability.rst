...
使用与 ``spatial_context`` 关联的平面实体，调用 :ref:`OpenXRSpatialEntityExtension.discover_spatial_entities()<class_OpenXRSpatialEntityExtension_method_discover_spatial_entities>` 和 :ref:`OpenXRSpatialEntityExtension.query_snapshot()<class_OpenXRSpatialEntityExtension_method_query_snapshot>` 方法。

\ ``component_data`` 是针对此平面功能需要发现的 :ref:`OpenXRSpatialComponentData<class_OpenXRSpatialComponentData>` 数据。
