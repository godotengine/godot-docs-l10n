...
被固定的物体试图保持彼此相对位置关系的力度。数值越高，力度越强。

\ **注意：** 仅在使用 GodotPhysics3D 时受支持。使用 Jolt Physics 时，该参数会被忽略。
...
被固定的物体试图保持彼此相对速度关系的力度。数值越高，力度越强。
...
如果该值大于 0，则它表示此 Joint3D 施加在其两端上的最大冲量（impulse）。
...
**注意：** 仅在使用 GodotPhysics3D 时支持。当使用 Jolt Physics 时，此参数将被忽略。
...
一旦超出滑块轴的限制范围，应用于其沿轴移动的一个系数。数值越低，移动速度越慢。
...
一旦超出限制范围后的回弹（恢复）程度。数值越低，速度能量损失得越多。
...
只要滑块处于限制范围内，就对其沿轴移动施加的一个系数。数值越低，移动速度越慢。
...
当滑块沿垂直于滑动轴的轴线发生运动时，产生的回弹（恢复）程度。
...
摆动（Swing）是指绕着垂直于扭转轴的轴线进行的左右旋转。

摆动跨度（swing span）定义了沿摆动轴有多少旋转量不会被纠正（即允许自由旋转的范围）。

可以将其理解为 :ref:`ConeTwistJoint3D<class_ConeTwistJoint3D>` 的松动程度。如果该值低于 0.05，此行为将被锁定。
...
扭转（Twist）是指绕着扭转轴进行的旋转，这个数值定义了关节可以扭转多远（即允许自由扭转的角度范围）。如果该值低于 0.05，扭转行为将被锁定。
...
关节扭转的容易程度。如果该值太低，就需要更大的力才能扭转关节。
...
当修正跨越各轴的旋转超出限制时，这个误差容忍因子决定了修正过程会被放慢多少。数值越低，放慢得越明显（修正得越慢）。
...
用于创建世界边界形状的常量（由 :ref:`WorldBoundaryShape3D<class_WorldBoundaryShape3D>` 资源使用）。
...
用于创建分离射线形状的常量（由 :ref:`SeparationRayShape3D<class_SeparationRayShape3D>` 资源使用）。
...
用于创建球体形状的常量（由 :ref:`SphereShape3D<class_SphereShape3D>` 资源使用）。
...
用于创建盒状形状的常量（由 :ref:`BoxShape3D<class_BoxShape3D>` 资源使用）。
...
用于创建胶囊体形状的常量（由 :ref:`CapsuleShape3D<class_CapsuleShape3D>` 资源使用）。
...
用于创建圆柱体形状的常量（由 :ref:`CylinderShape3D<class_CylinderShape3D>` 资源使用）。
...
用于创建凸多边形形状的常量（由 :ref:`ConvexPolygonShape3D<class_ConvexPolygonShape3D>` 资源使用）。
...
用于创建凹多边形（三角网格）形状的常量（由 :ref:`ConcavePolygonShape3D<class_ConcavePolygonShape3D>` 资源使用）。
...
用于创建高度图形状的常量（由 :ref:`HeightMapShape3D<class_HeightMapShape3D>` 资源使用）。
...
内部使用的常量，用于表示软体形状。任何尝试创建这类形状的操作都会产生错误。
...
内部使用的常量，用于表示自定义形状。在使用 Godot 物理引擎或 Jolt 物理引擎时，任何尝试创建这类形状的操作都会产生错误。
...
用于设置/获取活动状态的线性速度阈值。当一个物体的线性速度和角速度都被标记为低于（潜在的）休眠阈值时，在经过指定的时间后，它将会进入休眠状态。
...
用于设置/获取活动状态的角速度阈值。当一个物体的线性速度和角速度都被标记为低于（潜在的）休眠阈值时，在经过指定的时间后，它将会进入休眠状态。
...
在物理服务器中创建一个 3D 盒子形状，并返回标识它的 :ref:`RID<class_RID>`\ 。可使用 :ref:`shape_set_data()<class_PhysicsServer3D_method_shape_set_data>` 设置盒子的半边距。
...
在物理服务器中创建一个 3D 胶囊体形状，并返回标识它的 :ref:`RID<class_RID>`\ 。可使用 :ref:`shape_set_data()<class_PhysicsServer3D_method_shape_set_data>` 设置胶囊体的高度和半径。
...
在物理服务器中创建一个 3D 凹多边形形状，并返回标识它的 :ref:`RID<class_RID>`\ 。可使用 :ref:`shape_set_data()<class_PhysicsServer3D_method_shape_set_data>` 设置凹多边形的三角形。
...
在物理服务器中创建一个 3D 凸多边形形状，并返回标识它的 :ref:`RID<class_RID>`\ 。可使用 :ref:`shape_set_data()<class_PhysicsServer3D_method_shape_set_data>` 设置凸多边形的点。
...
在物理服务器中创建一个自定义形状，并返回用于标识它的 :ref:`RID<class_RID>`\ 。使用 :ref:`shape_set_data()<class_PhysicsServer3D_method_shape_set_data>` 方法设置形状数据。

\ **注意：**\ 内置物理服务器不支持自定义形状，因此在使用 Godot 物理或 Jolt 物理时调用该方法始终会产生错误。作为 GDExtension 实现的自定义物理服务器可能支持自定义形状。
...
在物理服务器中创建一个 3D 圆柱体形状，并返回标识它的 :ref:`RID<class_RID>`\ 。可使用 :ref:`shape_set_data()<class_PhysicsServer3D_method_shape_set_data>` 设置圆柱体的高度和半径。
...
在物理服务器中创建一个 3D 高度图形状，并返回标识它的 :ref:`RID<class_RID>`\ 。可使用 :ref:`shape_set_data()<class_PhysicsServer3D_method_shape_set_data>` 设置高度图的数据。
...
\ **注意：** 仅在使用 GodotPhysics3D 时支持。当使用 Jolt Physics 时，该方法始终返回 ``1``\ ，因为 Jolt Physics 不支持关节求解器优先级。
...
\ **注意：** 仅在使用 GodotPhysics3D 时支持。当使用 Jolt Physics 时，该方法无效，因为 Jolt Physics 不支持关节求解器优先级。
...
在物理服务器中创建一个 3D 分离射线形状，并返回标识它的 :ref:`RID<class_RID>`\ 。可使用 :ref:`shape_set_data()<class_PhysicsServer3D_method_shape_set_data>` 设置形状的 ``length`` 和 ``slide_on_slope`` 属性。
...
返回用于配置形状的形状数据，例如盒子的半边距或凹形（三角形网格）形状的三角形。有关每种情况下该数据的精确格式，请参阅 :ref:`shape_set_data()<class_PhysicsServer3D_method_shape_set_data>`\ 。
...
设置用于配置形状的形状数据。要传递的 ``data`` 取决于形状的类型（见 :ref:`shape_get_type()<class_PhysicsServer3D_method_shape_get_type>`\ ）：

- :ref:`SHAPE_WORLD_BOUNDARY<class_PhysicsServer3D_constant_SHAPE_WORLD_BOUNDARY>`\ ：一个 :ref:`Plane<class_Plane>`\ ，

- :ref:`SHAPE_SEPARATION_RAY<class_PhysicsServer3D_constant_SHAPE_SEPARATION_RAY>`\ ：一个字典，包含键 ``"length"``\ （值为 :ref:`float<class_float>` 类型）和键 ``"slide_on_slope"``\ （值为 :ref:`bool<class_bool>` 类型），

- :ref:`SHAPE_SPHERE<class_PhysicsServer3D_constant_SHAPE_SPHERE>`\ ：一个 :ref:`float<class_float>` 值，表示球体的半径，

- :ref:`SHAPE_BOX<class_PhysicsServer3D_constant_SHAPE_BOX>`\ ：一个 :ref:`Vector3<class_Vector3>`\ ，包含盒体的半边距，

- :ref:`SHAPE_CAPSULE<class_PhysicsServer3D_constant_SHAPE_CAPSULE>`\ ：一个字典，包含键 ``"height"`` 和 ``"radius"``\ （值为 :ref:`float<class_float>` 类型），

- :ref:`SHAPE_CYLINDER<class_PhysicsServer3D_constant_SHAPE_CYLINDER>`\ ：一个字典，包含键 ``"height"`` 和 ``"radius"``\ （值为 :ref:`float<class_float>` 类型），

- :ref:`SHAPE_CONVEX_POLYGON<class_PhysicsServer3D_constant_SHAPE_CONVEX_POLYGON>`\ ：一个 :ref:`PackedVector3Array<class_PackedVector3Array>`\ ，包含定义凸多边形的点（形状将是这些点的凸包），

- :ref:`SHAPE_CONCAVE_POLYGON<class_PhysicsServer3D_constant_SHAPE_CONCAVE_POLYGON>`\ ：一个字典，包含键 ``"faces"``\ （值为 :ref:`PackedVector3Array<class_PackedVector3Array>`\ ，长度可被 3 整除，因此每三个点构成一个面）和键 ``"backface_collision"``\ （值为 :ref:`bool<class_bool>` 类型），

- :ref:`SHAPE_HEIGHTMAP<class_PhysicsServer3D_constant_SHAPE_HEIGHTMAP>`\ ：一个字典，包含键 ``"width"`` 和 ``"depth"``\ （值为 :ref:`int<class_int>` 类型），以及键 ``"heights"``\ （值为长度为 ``width * depth`` 的 :ref:`float<class_float>` 数组（即 :ref:`PackedFloat32Array<class_PackedFloat32Array>`\ ，如果 Godot 使用 ``precision=double`` 选项编译，则为 :ref:`PackedFloat64Array<class_PackedFloat64Array>`\ ）），以及可选的键 ``"min_height"`` 和 ``"max_height"``\ （值为 :ref:`float<class_float>` 类型），

- :ref:`SHAPE_SOFT_BODY<class_PhysicsServer3D_constant_SHAPE_SOFT_BODY>`\ ：输入参数 ``data`` 被忽略并且该方法无效。

- :ref:`SHAPE_CUSTOM<class_PhysicsServer3D_constant_SHAPE_CUSTOM>`\ ：如果物理服务器支持自定义形状，则输入参数 ``data`` 将由自定义物理服务器进行解释。
...
在物理服务器中创建一个 3D 球体形状，并返回标识它的 :ref:`RID<class_RID>`\ 。可使用 :ref:`shape_set_data()<class_PhysicsServer3D_method_shape_set_data>` 设置球体的半径。
...
在物理服务器中创建一个 3D 世界边界形状，并返回标识它的 :ref:`RID<class_RID>`\ 。可使用 :ref:`shape_set_data()<class_PhysicsServer3D_method_shape_set_data>` 设置形状的法线方向和距离属性。
