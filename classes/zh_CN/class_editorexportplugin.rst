...
**EditorExportPlugin** 会在用户导出项目时自动调用。它们最常见的用途是确定哪些文件应该包含在导出的项目中。对于每个插件，导出过程开始时都会调用 :ref:`_export_begin<class_EditorExportPlugin_method__export_begin>`\ ，然后会为每一个导出的文件调用 :ref:`_export_file<class_EditorExportPlugin_method__export_file>`\ 。
...
启用后，\ :ref:`_get_customization_configuration_hash<class_EditorExportPlugin_method__get_customization_configuration_hash>`\ 、\ :ref:`_customize_resource<class_EditorExportPlugin_method__customize_resource>` 和 :ref:`_customize_scene<class_EditorExportPlugin_method__customize_scene>` 将被调用并且必须被实现。
...
如果 :ref:`_begin_customize_resources<class_EditorExportPlugin_method__begin_customize_resources>` 返回 ``true``\ ，则需要实现该方法。
...
如果 :ref:`_get_export_options<class_EditorExportPlugin_method__get_export_options>` 的结果发生了改变，与 ``platform`` 对应的导出预设的选项需要更新，则返回 ``true``\ 。
...
在 :ref:`_export_file<class_EditorExportPlugin_method__export_file>` 中调用时，如果 ``remap`` 为 ``true``\ ，则当前文件将不会被导出，而是被重新映射到这个自定义文件。在其他地方调用时会忽略 ``remap``\ 。
...
将动态库（\*.dylib、\*.framework）添加到iOS的Xcode项目的链接阶段，并将其嵌入到生成的二进制文件中。
...
返回 :ref:`_get_export_options<class_EditorExportPlugin_method__get_export_options>` 提供的导出选项的当前值。
