...
如果设置为非 :ref:`TextServer.AUTOWRAP_OFF<class_TextServer_constant_AUTOWRAP_OFF>` 的值，文本将在节点的边界矩形内进行换行。如果你调整节点的大小，它将自动改变高度以显示所有文本。

\ **注意：** 启用了自动换行的标签必须配置自定义的最大宽度才能正常工作，这可以通过标签自身的 :ref:`Control.custom_maximum_size<class_Control_property_custom_maximum_size>` 实现，或者作为启用了 :ref:`Control.propagate_maximum_size<class_Control_property_propagate_maximum_size>` 的父控件传递过来的最大尺寸的结果。
