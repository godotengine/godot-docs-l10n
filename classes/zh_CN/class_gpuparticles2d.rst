...
\ ``process_time`` 定义了粒子在发射（emitting）开启状态下将要模拟的时间。\ ``process_time_residual`` 定义了粒子在发射关闭状态下，为了维持模拟效果将要额外处理的时间。当把 :ref:`speed_scale<class_GPUParticles2D_property_speed_scale>` 设置为 ``0.0`` 并结合使用时，这个方法非常适用于对粒子系统的时间轴进行‘搜寻’（seek，即跳转到特定时间点）。
