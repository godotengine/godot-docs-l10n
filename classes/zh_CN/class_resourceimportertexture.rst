...
你在网上可以找到的一些 HDR 图像可能已损坏，并且包含使用非线性 sRGB 传输函数（而不是使用线性编码）编码的数据。建议不要使用这些文件。如果你确实需要使用，启用 :ref:`process/hdr_as_srgb<class_ResourceImporterTexture_property_process/hdr_as_srgb>` 选项可以使它们看起来正确。
