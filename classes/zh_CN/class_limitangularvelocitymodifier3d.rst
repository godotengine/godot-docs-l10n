...
你可以通过指定根骨骼和末端骨骼来添加骨骼链，然后将它们之间的骨骼添加到一个列表中。根据 :ref:`exclude<class_LimitAngularVelocityModifier3D_property_exclude>`\ （排除）选项的设置，修饰器会处理这个列表中的骨骼，或者处理除了该列表之外的其他骨骼。
