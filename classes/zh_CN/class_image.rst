...
创建一个给定大小和格式的空图像。如果 ``use_mipmaps`` 为 ``true``\ ，则为该图像生成 Mipmap。
...
\ **注意：**\ TinyEXR 模块在非编辑器构建中被禁用，这意味着当 :ref:`save_exr_to_buffer()<class_Image_method_save_exr_to_buffer>` 从导出的项目中被调用时将返回一个空字节数组。
