...
断言条件 ``condition`` 为 ``true``\ 。如果条件 ``condition`` 为 ``false`` ，则会生成错误，且当前方法将返回一个默认值。当从编辑器运行时，断言失败还会触发调试器中断。该函数可以作为 :ref:`@GlobalScope.push_error()<class_@GlobalScope_method_push_error>` 的加强版使用，用于向项目开发者和插件用户报错。
...
\ ``type`` 可以不是常量，这一点与 ``is`` 的右操作数不同，\ ``is`` 运算符支持的功能更多（例如类型化数组和字典）。如果你不需要动态地检查类型，请使用该运算符，不要使用此方法。
...
另见 :ref:`@GlobalScope.typeof()<class_@GlobalScope_method_typeof>`\ 、\ :ref:`Object.is_class()<class_Object_method_is_class>`\ 、\ :ref:`Object.get_script()<class_Object_method_get_script>`\ 、\ :ref:`Array.is_same_typed()<class_Array_method_is_same_typed>`\ （以及其他 :ref:`Array<class_Array>` 方法），\ :ref:`Dictionary.is_same_typed()<class_Dictionary_method_is_same_typed>`\ （以及其他 :ref:`Dictionary<class_Dictionary>` 方法）。
...
**已弃用：** Use :ref:`ClassDB.class_exists()<class_ClassDB_method_class_exists>` instead.
