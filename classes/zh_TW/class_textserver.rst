...
將數字從阿拉伯數字（0..9）轉換為 ``language`` 語言的記數系統。

如果省略 ``language``\ ，則會使用啟動的區域設定。
...
\ **注意：**\ 如果伺服器支援 :ref:`FEATURE_CONTEXT_SENSITIVE_CASE_CONVERSION<class_TextServer_constant_FEATURE_CONTEXT_SENSITIVE_CASE_CONVERSION>` 功能（\ :ref:`TextServerAdvanced<class_TextServerAdvanced>` 支持），則大小寫取決於區域設定，並且對本文敏感。
