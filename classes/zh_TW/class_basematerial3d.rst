...
頂點色視為以 sRGB 色域儲存，算繪時會轉換到線性色域。另見 :ref:`vertex_color_is_srgb<class_BaseMaterial3D_property_vertex_color_is_srgb>`\ 。
...
強制著色器將反照率由 sRGB 轉為線性空間。另見 :ref:`albedo_texture_force_srgb<class_BaseMaterial3D_property_albedo_texture_force_srgb>`\ 。
...
若為 ``true``\ ，強制將 :ref:`albedo_texture<class_BaseMaterial3D_property_albedo_texture>` 從 sRGB 色域轉換為線性色域。另見 :ref:`vertex_color_is_srgb<class_BaseMaterial3D_property_vertex_color_is_srgb>`\ 。

此選項僅在必要時啟用（通常於將 :ref:`ViewportTexture<class_ViewportTexture>` 用作 :ref:`albedo_texture<class_BaseMaterial3D_property_albedo_texture>` 時）。若不該開啟卻開啟，紋理會顯得過暗；反之亦然則會顯得過亮。
...
若設為 ``true``\ ，視頂點色為 sRGB，渲染時轉為線性；若為 ``false`` 則視為線性色並直接使用。另見 :ref:`albedo_texture_force_srgb<class_BaseMaterial3D_property_albedo_texture_force_srgb>`\ 。
...
若指定的 :ref:`Feature<enum_BaseMaterial3D_Feature>` 已啟用，返回 ``true``\ 。
...
返回與指定 :ref:`TextureParam<enum_BaseMaterial3D_TextureParam>` 關聯的 :ref:`Texture2D<class_Texture2D>`\ 。
...
若為 ``true``\ ，啟用指定的 :ref:`Feature<enum_BaseMaterial3D_Feature>`\ 。\ **BaseMaterial3D** 中的許多功能必須先啟用才能使用，藉此僅在需要時才付出效能成本。亦可透過將對應屬性設為 ``true`` 來啟用。
