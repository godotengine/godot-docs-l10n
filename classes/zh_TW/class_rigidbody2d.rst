...
如果位 ``true``\ ，則物體被凍結。不再施加重力和力。

請參閱 :ref:`freeze_mode<class_RigidBody2D_property_freeze_mode>`\ ，以設定凍結時，物體的行為。

對於始終凍結的物體，請改用 :ref:`StaticBody2D<class_StaticBody2D>` 或 :ref:`AnimatableBody2D<class_AnimatableBody2D>`\ 。
