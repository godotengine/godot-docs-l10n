...
連接兩個位於 :ref:`NavigationRegion3D<class_NavigationRegion3D>` 上的位置的連結，導覽時能夠讓代理走這個鏈接。
...
連接兩個位於 :ref:`NavigationRegion3D<class_NavigationRegion3D>` 上的位置的連結，導覽時能夠讓代理走這個鏈接。這兩個位置可以在同一個 :ref:`NavigationRegion3D<class_NavigationRegion3D>` 上，也可以是在兩個不同的區塊上。連結可以用來表達沿著導覽網格表面行進以外的導覽方法，例如滑鎖、傳送、跳過溝壑等等。
...
返回這個連接是可以雙向通行，還是只能從 :ref:`start_position<class_NavigationLink3D_property_start_position>` 通向 :ref:`end_position<class_NavigationLink3D_property_end_position>`\ 。
...
該連結目前是否處於活動狀態。如果為 ``false`` ，則 :ref:`NavigationServer3D.map_get_path()<class_NavigationServer3D_method_map_get_path>` 會忽略這個連結。
...
連結搜索的距離由 :ref:`NavigationServer3D.map_set_link_connection_radius()<class_NavigationServer3D_method_map_set_link_connection_radius>` 控制。
...
當尋路從其他地區的導覽網格進入該連結時，\ :ref:`enter_cost<class_NavigationLink3D_property_enter_cost>` 會加到路徑距離中，用於確定最短路徑。
...
決定連結所屬導覽層的位元域。使用 :ref:`NavigationServer3D.map_get_path()<class_NavigationServer3D_method_map_get_path>` 請求路徑時會檢查這些導覽層。
...
當尋路沿著該連結移動時，移動距離會和 :ref:`travel_cost<class_NavigationLink3D_property_travel_cost>` 相乘，用於確定最短路徑。
...
返回該連結的 :ref:`end_position<class_NavigationLink3D_property_end_position>` 的全域位置。
...
返回該連結的 :ref:`start_position<class_NavigationLink3D_property_start_position>` 的全域位置。
...
返回 :ref:`navigation_layers<class_NavigationLink3D_property_navigation_layers>` 位元遮罩中指定的層是否啟用，給定的 ``layer_number`` 應在 1 和 32 之間。
...
設定該連結的 :ref:`end_position<class_NavigationLink3D_property_end_position>` 的全域位置。
...
設定該連結的 :ref:`start_position<class_NavigationLink3D_property_start_position>` 的全域位置。
...
根據 ``value``\ ，啟用或禁用 :ref:`navigation_layers<class_NavigationLink3D_property_navigation_layers>` 位元遮罩中指定的層，給定的 ``layer_number`` 應在 1 和 32 之間。
