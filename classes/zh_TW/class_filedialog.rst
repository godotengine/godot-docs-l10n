...
**FileDialog** 是用來選擇檔案系統中檔和目錄的預設對話方塊。支援篩檢程式遮罩。\ **FileDialog** 會根據 :ref:`file_mode<class_FileDialog_property_file_mode>` 自動設定視窗的標題。如果你想使用自訂標題，請將 :ref:`mode_overrides_title<class_FileDialog_property_mode_overrides_title>` 設定為 ``false``\ ，禁用此功能。
