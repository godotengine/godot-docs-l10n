...
\ **注意：**\ 如果你只是想要知道該字串中是否包含 ``what``\ ，請使用 :ref:`contains()<class_StringName_method_contains>`\ 。在 GDScript 中，你還可以使用 ``in`` 運算子。
...
返回這個字串中 ``what`` **最後一次**\ 出現時的索引，不存在時則為 ``-1``\ 。搜索的起點可以用 ``from`` 指定，終點為該字串的末尾。這個方法與 :ref:`find()<class_StringName_method_find>` 相對。
