...
**StreamPeerTCP**\ 的初始狀態。這也是斷開連接後的狀態。
...
輪詢通訊端，更新其狀態。見 :ref:`get_status()<class_StreamPeerTCP_method_get_status>`\ 。
