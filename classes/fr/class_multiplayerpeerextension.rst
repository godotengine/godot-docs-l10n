...
Classe qui peut être héritée pour implémenter des couches de réseau API multijoueur personnalisées via GDExtension.
...
Appelée lorsque le statut "is server" ("est serveur") est demandé sur la :ref:`MultiplayerAPI<class_MultiplayerAPI>`. Voir :ref:`MultiplayerAPI.is_server()<class_MultiplayerAPI_method_is_server>`.
...
Appelée pour vérifier si le serveur peut agir comme un relais dans la configuration actuelle. Voir :ref:`MultiplayerPeer.is_server_relay_supported()<class_MultiplayerPeer_method_is_server_relay_supported>`.
