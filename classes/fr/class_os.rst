...
Renvoie le chemin de fichier vers l'exécutable actuel du moteur.

\ **Note :** Sur macOS, si vous voulez lancer une autre instance de Godot, utilisez toujours :ref:`create_instance()<class_OS_method_create_instance>` au lieu de vous baser sur le chemin de l'exécutable.
...
Renvoie le nombre de cœurs *logiques* du CPU disponibles sur la machine hôte. Sur les processeurs avec le mode HyperThreading activé, ce nombre sera supérieur au nombre de cœurs *physiques* du CPU.
...
Renvoie l'identifiant du fil d'exécution actuel. Cela peut être utile dans les journaux pour faciliter le débogage des applications à plusieurs fils d'exécution.

\ **Note :** Ces identifiants ne sont pas déterministes et peuvent être réutilisés durant plusieurs lancement de l'application.
...
Déplace le fichier ou le répertoire au chemin ``path`` donné vers la corbeille du système. Voir aussi :ref:`DirAccess.remove()<class_DirAccess_method_remove>`.

La méthode ne prend que des chemins globaux, donc vous pouvez avoir besoin d'utiliser :ref:`ProjectSettings.globalize_path()<class_ProjectSettings_method_globalize_path>`. Ne l'utilisez pas pour les fichiers dans ``res://`` car elle ne fonctionnera pas dans les projets exportés.

Renvoie :ref:`@GlobalScope.FAILED<class_@GlobalScope_constant_FAILED>` si le fichier ou le répertoire ne peut pas être trouvé, ou si le système ne supporte pas cette méthode.
...
\ **Note :** Cette méthode est implémentée sur Android, Linux, macOS et Windows.

\ **Note :** Si l'utilisateur a désactivé la corbeille sur son système, le fichier sera définitivement supprimé à la place.
