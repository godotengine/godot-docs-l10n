...
Représente la taille de l’enum :ref:`FontLCDSubpixelLayout<enum_TextServer_FontLCDSubpixelLayout>`.
...
Le texte de gauche à droite est écrit verticalement de haut en bas.

Le texte de droite à gauche est écrit verticalement de bas en haut.
...
Le retour à la ligne automatique est désactivé.
...
**Obsolète :** Use ``BREAK_TRIM_START_EDGE_SPACES | BREAK_TRIM_END_EDGE_SPACES`` instead.
...
Désactive le "font hinting" (plus lisse mais moins net).
...
Utiliser le mode d'hinting de police léger.
...
TextServer supporte des dispositions de texte simples.
...
TextServer supporte des dispositions de texte bidirectionnelles.
...
TextServer supporte les dispositions verticales.
...
TextServer supporte le text shaping complexe.
...
TextServer supporte le chargement de polices matricielles.
...
TextServer supporte le chargement de polices du système.
...
L'espacement pour le caractère d'espace.
...
Représente la taille de l'énumération :ref:`SpacingType<enum_TextServer_SpacingType>`.
...
La police est en italique ou en oblique.
...
Dessine une boîte qui affiche un code hexadécimal de caractère. Utilisé pour remplacer les caractères manquants.
...
Renvoie le mode d'anticrénelage de la police.
...
**Obsolète :** Use :ref:`Viewport<class_Viewport>` oversampling, or the ``oversampling`` argument of the ``draw_*`` methods instead.
...
Renvoie le nom de la famille de police.
...
Renvoie les drapeaux de style de police.
...
Renvoie le nom du style de police.
...
Renvoie une chaîne contenant tous les caractères disponibles dans la police.
...
Renvoie ``true`` si les polices du système peuvent être automatiquement utilisées comme des repli.
...
**Obsolète :** Use :ref:`TranslationServer.format_number()<class_TranslationServer_method_format_number>` instead.
...
Renvoie ``true`` si la langue va de droite à gauche.
...
**Obsolète :** Use :ref:`TranslationServer.parse_number()<class_TranslationServer_method_parse_number>` instead.
...
**Obsolète :** Use :ref:`TranslationServer.get_percent_sign()<class_TranslationServer_method_get_percent_sign>` instead.
