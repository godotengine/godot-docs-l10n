...
Émis après qu'une préférence de l'éditeur a changé.
...
Émis après que n'importe quel réglage de l'édiateur a changé. Il est utilisé par divers greffons de l'éditeurs pour mettre à jour leur affichage lors de changements de thème ou de configuration.
...
La couleur du manipulateur de l'éditeur 3D utilisée pour les nœuds :ref:`CPUParticles3D<class_CPUParticles3D>` et :ref:`GPUParticles3D<class_GPUParticles3D>`.
...
La couleur d'un port/connection de type Vector2.
...
La couleur d'un port/connection de type Vector3.
...
La couleur d'un port/connection de type Vector4.
...
Si ``false``, l'éditeur enregistrera toutes les scènes lors de la confirmation de l'action **Sauvegarder** lorsqu'on quitte l'éditeur ou quitte vers la liste des projets. Si ``true``, l'éditeur demandera à sauvegarder chaque scène individuellement.
...
Si ``true``, les scènes et les scripts sont sauvegardés lorsque l'éditeur perd le focus. Selon le workflow, ce comportement peut être moins intrusif que :ref:`text_editor/behavior/files/autosave_interval_secs<class_EditorSettings_property_text_editor/behavior/files/autosave_interval_secs>` ou de se souvenir d'enregistrer manuellement.
...
Si ``true``, dessine les caractères de tabulation comme des chevrons.
...
La couleur du curseur de l'éditeur de script.
...
Renvoie la liste des fichiers et répertoires favoris pour ce projet.
...
Renvoie la liste des dossiers récemment visités dans le dialogue des fichiers de ce projet.
...
Définit la liste des fichiers et dossiers favoris pour ce projet.
...
Définit des métadonnées spécifiques au projet dans la section ``section``, la clé ``key`` et les données ``data`` spécifiées. Ces métadonnées sont enregistrées à l'extérieur du dossier du projet, et ne seront donc pas dans le système de contrôle de version. Voir aussi :ref:`get_project_metadata()<class_EditorSettings_method_get_project_metadata>`.
...
Définit une liste des dossiers récemment visités dans le dialogue de fichiers de ce projet.
