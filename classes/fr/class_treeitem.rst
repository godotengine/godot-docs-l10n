...
Si ``true``, le TreeItem est développé.
...
La hauteur minimale personnalisée.
...
Si ``true``, la réduction est désactivée pour ce TreeItem.
...
Rétablit la couleur d'arrière-plan par défaut de la colonne spécifiée.
...
Rétablit la couleur de la colonne spécifiée à la valeur par défaut.
...
Désélectionne la colonne donnée.
...
Renvoie le mode de traduction automatique de la colonne.
...
Renvoie le mode des cellules de la colonne.
...
Renvoie la couleur personnalisée de la colonne ``column``.
...
Renvoie ``true`` si ``expand_right`` est défini.
...
Renvoie le premier enfant du TreeItem.
...
Renvoie la :ref:`Color<class_Color>` modulant l'icône de la colonne.
...
Renvoie la valeur d'une colonne :ref:`CELL_MODE_RANGE<class_TreeItem_constant_CELL_MODE_RANGE>`.
...
Renvoie un dictionnaire contenant les paramètres de plage pour une colonne donnée. Les clés sont "min", "max", "step", et "expr".
...
Obtient le suffixe affiché après la valeur de la colonne.
...
Renvoie le texte de la colonne donnée.
...
Renvoie l'alignement du texte de la colonne donnée.
...
Définit la couleur personnalisée de la colonne donnée.
...
**Obsolète :** Use :ref:`set_custom_draw_callback()<class_TreeItem_method_set_custom_draw_callback>` instead.
...
Module l'icône de la colonne donnée avec ``modulate``.
...
Définit la valeur des métadonnées pour la colonne donnée, qui peut être récupérée ultérieurement en utilisant :ref:`get_metadata()<class_TreeItem_method_get_metadata>`. Cela peut être utilisé, par exemple, pour stocker une référence aux données d'origine.
...
Définit la valeur d'une colonne :ref:`CELL_MODE_RANGE<class_TreeItem_constant_CELL_MODE_RANGE>`.
...
Définit un texte à afficher après la valeur d'une colonne (par exemple, une abréviation d'unité).
...
Définit le texte pour la colonne donnée.
...
Définit le texte de l’info-bulle de la colonne donnée.
