...
Le serveur d'affichage supporte le changement de l'orientation de l'écran. **Android, iOS**
...
Élément de cellule d'arbre/de tableau.
...
Élément de ligne d'arbre/de tableau.
...
Valeur maximale de :ref:`MouseMode<enum_DisplayServer_MouseMode>`.
...
Orientation en mode paysage par défaut.
...
Orientation paysage ou portrait automatique (par défaut ou inversé selon le capteur).
...
Représente la taille de l'énumération :ref:`CursorShape<enum_DisplayServer_CursorShape>`.
...
\ **Note :** Cette option est uniquement implémentée sous Windows (11).
...
Représente la taille de l'énumération :ref:`WindowResizeEdge<enum_DisplayServer_WindowResizeEdge>`.
...
- Linux (X11) : ``X11::Display*`` pour l'affichage.

- Linux (Wayland) : ``wl_display*`` pour l'affichage.

- Android : ``EGLDisplay`` pour l'affichage.
...
- Windows : ``HWND`` pour la fenêtre.

- Linux (X11) : ``X11::Window*`` pour la fenêtre.

- Linux (Wayland) : ``wl_surface`` pour la fenêtre.

- macOS : ``NSWindow*`` pour la fenêtre.

- iOS : ``UIViewController*`` pour le contrôleur de vue.

- Android : ``jObject`` pour l'activité.
...
- Windows : ``HDC`` pour la fenêtre (seulement avec le moteur de rendu Compatibilité).

- macOS : ``NSView*`` pour la fenêtre de vue principale.

- iOS : ``UIView*`` pour la fenêtre de vue principale.
...
Contexte OpenGL (seulement avec le moteur de rendu Compatibilité) :

- Windows : ``HGLRC`` pour la fenêtre (GL natif), ou ``EGLContext`` pour la fenêtre (ANGLE).

- Linux (X11) : ``GLXContext*`` pour la fenêtre.

- Linux (Wayland) : ``EGLContext`` pour la fenêtre.

- macOS : ``NSOpenGLContext*`` pour la fenêtre (GL natif), ou ``EGLContext`` pour la fenêtre (ANGLE).

- Android : ``EGLContext`` pour la fenêtre.
...
- Windows : ``EGLDisplay`` pour la fenêtre (ANGLE).

- macOS : ``EGLDisplay`` pour la fenêtre (ANGLE).

- Linux (Wayland) : ``EGLDisplay`` pour la fenêtre.
...
- Windows : ``EGLConfig`` pour la fenêtre (ANGLE).

- macOS : ``EGLConfig`` pour la fenêtre (ANGLE).

- Linux (Wayland) : ``EGLConfig`` pour la fenêtre.
...
Renvoie ``1`` si un thème d'interface utilisateur à contraste élevé doit être utilisé, ``0`` sinon. Renvoie ``-1`` si le statut est inconnu.

\ **Note :** Cette méthode est implémentée sur Linux (X11/Wayland, GNOME), macOS et Windows.
...
Renvoie ``1`` si les flashs, les clignotements et les autres contenus mouvants qui peuvent causer des saisies chez les utilisateurs d'épilepsie photosensible doivent être désactivés, ``0`` sinon. Renvoie ``-1`` si le statut est inconnu.
...
Renvoie ``1`` si les images d'arrière-plan, la transparence et les autres fonctionnalités qui peuvent réduire le contraste entre le premier plan et l'arrière-plan doivent être désactivées, ``0`` sinon. Renvoie ``-1`` si le statut est inconnu.
...
Ajoute un élément d'accessibilité enfant.

\ **Note : ** Les enfants :ref:`Node<class_Node>` et les sous-éléments sont automatiquement ajoutés à la liste des enfants.
...
Ajoute du support pour une action d'accessibilité personnalisée. ``action_id`` est passé comme un argument au callback de l'action :ref:`ACTION_CUSTOM<class_DisplayServer_constant_ACTION_CUSTOM>`.
...
Ajoute un élément qui est contrôlé par cet élément.
...
Ajoute un élément qui décrit cet élément.
...
Ajoute un élément qui détaille cet élément.
...
Définit l'état coché de l'élément.
...
Défini le nom de classe de l'élément.
...
Définit la description d'accessibilité de l'élément.
...
Définit l'élément possédant actuellement le focus.
...
Définit la langue du texte de l'élément.
...
Définit la position de l'élément dans la liste.
...
Définit le niveau hiérarchique de l'élément dans la liste.
...
Définit l'orientation des éléments de la liste.
...
Définit le nom d'accessibilité de l'élément.
...
Définit l'élément suivant sur la ligne.
...
Définit la plage de valeur numérique.
...
Définit le pas de valeur numérique.
...
Définit la position de la ligne dans la table.
...
Définit l'alignement du texte de l'élément.
...
Définit la sélection de texte au champ texte. ``text_start_id`` et ``text_end_id`` devraient être des éléments créés par :ref:`accessibility_create_sub_text_edit_elements()<class_DisplayServer_method_accessibility_create_sub_text_edit_elements>`. Les décalages de caractère sont relatifs à l'élément correspondant.
...
Définit la transformation 2D de l'élément.
...
Définit la valeur du texte de l'élément.
...
Joue le son du bip du système d'exploitation. si possible. Parce qu'il vient de l'OS, le son du bip sera audible même si l'application est mise en sourdine. Il peut également être désactivé pour l'ensemble de l'OS par l'utilisateur.

\ **Note :** Cette méthode est mise implémentée sur macOS, Linux (X11/Wayland) et Windows.
...
Renvoie le presse-papiers de l'utilisateur en tant que chaîne si possible.
...
Renvoie ``true`` s'il y a un contenu texte dans le presse-papier de l'utilisateur.
...
Renvoie ``true`` s'il y a un contenu d'image dans le presse-papiers de l'utilisateur.
...
Définit le contenu du presse-papiers de l'utilisateur à la chaîne donnée.
...
Permet au PID ``process_id`` de voler le focus de cette fenêtre. En d'autres termes, cela désactive la protection de vol de focus du système d'exploitation pour le PID spécifié.

\ **Note :** Cette méthode n'est implémentée que sur Windows.
...
Renvoie ``true`` si un clavier matériel est connecté.

\ **Note :** Cette méthode est implementée sur Android et iOS. Sur les autres plateformes, cette méthode renvoie toujours ``true``.
...
Renvoie ``true`` si l'OS prend en charge le mode sombre.

\ **Note :** Cette méthode est implémentée sur Android, iOS, macOS, Windows et Linux (X11/Wayland).
...
Renvoie le mode de souris actuel. Voir aussi :ref:`mouse_set_mode()<class_DisplayServer_method_mouse_set_mode>`.
...
Définit le mode de souris actuel. Voir aussi :ref:`mouse_get_mode()<class_DisplayServer_method_mouse_get_mode>`.
...
Renvoie le plus grand facteur d'échelle de tous les écrans.

\ **Note :** Sur macOS, la valeur renvoyée est ``2.0`` s'il y a au moins un écran hiDPI (Retina) dans le système, et ``1.0`` dans tous les autres cas.

\ **Note :** Cette méthode est implémentée seulement sur macOS.
...
Cache le clavier virtuel s’il est montré, ne fait rien autrement.
