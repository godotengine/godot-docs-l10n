...
Importe une image pour utiliser dans le rendu 2D ou 3D.
...
Cet importeur importe des ressources :ref:`CompressedTexture2D<class_CompressedTexture2D>`. Si vous devez traiter l'image dans des scripts d'une manière plus pratique, utilisez :ref:`ResourceImporterImage<class_ResourceImporterImage>` à la place. Voir aussi :ref:`ResourceImporterLayeredTexture<class_ResourceImporterLayeredTexture>`.
...
**Obsolète :** The same result can be achieved by setting :ref:`process/channel_remap/green<class_ResourceImporterTexture_property_process/channel_remap/green>` to ``Green Inverted``.
