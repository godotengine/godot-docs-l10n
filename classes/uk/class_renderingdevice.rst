...
Дозволяє використовувати цей буфер як вхідні дані для операції побудови структури прискорення. Спочатку необхідно перевірити, чи підтримує це графічний процесор:
...
Дозволяє оновлювати структуру прискорення після її створення.
...
Дозволяє стиснути структуру прискорення для зменшення обсягу використовуваної пам'яті після її побудови.
...
Під час побудови структури прискорення надає пріоритет продуктивності обходу променів над продуктивністю побудови.
...
Під час побудови структури прискорення надає пріоритет продуктивності компіляції над продуктивністю обходу променів.
...
Зменшує обсяг пам'яті, що використовується структурою прискорення, що може призвести до зниження продуктивності обчислення траєкторій променів.
...
Непрозора геометрія не викликає жодних шейдерів влучання.
...
Ця геометрія викликає шейдер «any hit» лише один раз для кожного примітиву.
...
Вимкнути відсіювання трикутних граней для цього екземпляра під час обчислення траєкторій променів.
...
Під час обходу променів змінює напрямок трикутника для цього екземпляра.
...
Змушує всі геометрії в цьому екземплярі розглядатися як непрозорі, що запобігає виклику будь-яких шейдерів зіткнення.
...
Змушує всі геометрії в цьому екземплярі розглядатися як непрозорі, що дозволяє викликати будь-які шейдери зіткнення.
...
Однорідність структури прискорення.
...
Етап шейдера генерації променів. Його можна використовувати для генерації первинних променів.
...
Будь-який етап шейдера зіткнення. Викликається, коли точки перетину променів не є непрозорими. Цю функцію можна використовувати для визначення дій, що відбуваються при зіткненні променя з будь-яким елементом геометрії сцени.
...
Етап шейдера «Найближче зіткнення». Цю опцію можна використовувати для визначення того, що відбуватиметься, коли промінь зіткнеться з найближчою геометрією у сцені.
...
Етап пропуску шейдера. Цей параметр дозволяє вказати, що відбуватиметься, якщо промінь не потрапить на жодний об’єкт у сцені.
...
Етап шейдера перетину. Вбудований шейдер перетину для трикутників. Його можна використовувати для обчислення перетинів променів із примітивами, що не є трикутниками.
...
Біт етапу шейдера генерації променів (див. також :ref:`SHADER_STAGE_RAYGEN<class_RenderingDevice_constant_SHADER_STAGE_RAYGEN>`).
...
Будь-який біт етапу шейдера, що спрацьовує (див. також :ref:`SHADER_STAGE_ANY_HIT<class_RenderingDevice_constant_SHADER_STAGE_ANY_HIT>`).
...
Біт етапу шейдера з найближчим влученням (див. також :ref:`SHADER_STAGE_CLOSEST_HIT<class_RenderingDevice_constant_SHADER_STAGE_CLOSEST_HIT>`).
...
Пропущений біт етапу шейдера (див. також :ref:`SHADER_STAGE_MISS<class_RenderingDevice_constant_SHADER_STAGE_MISS>`).
...
Біт етапу шейдера перетину (див. також :ref:`SHADER_STAGE_INTERSECTION<class_RenderingDevice_constant_SHADER_STAGE_INTERSECTION>`).
...
Підтримка розширення для запитів на променевий трасування.



\ **Примітка:** Наразі ця функція підтримується лише при використанні Vulkan. Вона не підтримується в macOS та iOS (навіть на апаратному забезпеченні, що підтримує променеве трасування) через обмеження MoltenVK.
...
Підтримка розширення конвеєра трасування променів.



\ **Примітка:** Наразі ця функція підтримується лише при використанні Vulkan. Вона не підтримується в macOS та iOS (навіть на апаратному забезпеченні, що підтримує трасування променів) через обмеження MoltenVK.
...
Підтримка виведення зображення з високим динамічним діапазоном (HDR).
...
Створює нову структуру прискорення нижнього рівня (BLAS). Доступ до неї можна отримати за допомогою повернутого ідентифікатора RID.

Після завершення роботи з RID слід звільнити його за допомогою методу :ref:`free_rid()<class_RenderingDevice_method_free_rid>` класу RenderingDevice.
...
Створює нову таблицю прив'язки шейдерів (SBT). Доступ до неї можна отримати за допомогою повернутого ідентифікатора RID.

Після завершення роботи з RID слід звільнити його за допомогою методу :ref:`free_rid()<class_RenderingDevice_method_free_rid>` класу RenderingDevice.

Він буде звільнений автоматично, коли буде звільнено ``raytracing_pipeline``.

Таблиця SBT для влучень змінює свій розмір за потреби. ``initial_hit_group_capacity`` використовується для виділення початкової пам'яті для резервного копіювання.
...
Виділяє суцільний діапазон записів SBT з ``hit_sbt``.

Повернуте значення слід присвоїти :ref:`RDAccelerationStructureInstance.hit_sbt_range<class_RDAccelerationStructureInstance_property_hit_sbt_range>`.

Під час проходження променя індекс групи влучень обчислюється за формулою:

 (індекс геометрії в :ref:`RDAccelerationStructureInstance.blas<class_RDAccelerationStructureInstance_property_blas>`)

× (крок SBT, що використовується в ``traceRayEXT``)

 + (зсув SBT, що використовується в ``traceRayEXT``)

 + (зсув діапазону)

\ ``hit_group_count`` має бути достатньо великим, щоб охопити всі записи SBT, які можуть бути індексовані за цим рівнянням. Зазвичай це відповідає:

 (кількість геометрії в :ref:`RDAccelerationStructureInstance.blas<class_RDAccelerationStructureInstance_property_blas>`)

 × (крок SBT, що використовується в ``traceRayEXT``)

Виділений діапазон не ініціалізований і повинен бути заповнений за допомогою :ref:`hit_sbt_range_update()<class_RenderingDevice_method_hit_sbt_range_update>`.
...
Звільняє діапазон SBT, який раніше було виділено за допомогою функції :ref:`hit_sbt_range_alloc()<class_RenderingDevice_method_hit_sbt_range_alloc>`.

Після звільнення цей діапазон не повинен використовуватися жодною структурою прискорення.
...
Оновлює вміст діапазону SBT, що відповідає результату пошуку.

\ ``hit_group_indices`` визначає індекси в масиві груп результатів, наданому в :ref:`raytracing_pipeline_create()<class_RenderingDevice_method_raytracing_pipeline_create>`.

Параметр ``offset`` визначає, з якої позиції у виділеному діапазоні починається запис. Це дозволяє здійснювати часткове оновлення діапазону. Однак перед використанням у диспетчеризації трасування променів весь діапазон має бути повністю ініціалізований.
...
Встановлює новий ``raytracing_pipeline`` для ``hit_sbt``.

Новий конвеєр повинен бути надмножиною попереднього. Існуючі групи влучень повинні зберігати той самий порядок, а нові групи влучень слід додавати в кінець. Це дозволяє зберегти існуючі записи SBT.

Попередній конвеєр повинен залишатися чинним протягом виклику.
...
Запускає список команд трасування променів. Повернуте значення слід передавати іншим функціям ``raytracing_list_*``.

Одночасно не можна створювати кілька списків трасування променів; спочатку потрібно завершити попередній список за допомогою методу :ref:`raytracing_list_end()<class_RenderingDevice_method_raytracing_list_end>`.

Проста операція трасування променів може виглядати так (код не є повним прикладом):
...
.. tabs::
 ...
 .. code-tab:: gdscript
    ...
    # Створити структури прискорення.
    rd.blas_build(blas)
    ...
    # Прив'язати конвеєр та уніформи.
...
Прив'язує ``raytracing_pipeline`` до вказаного ``raytracing_list``.
...
Прив'язує ``uniform_set`` до цього ``raytracing_list``.
...
Завершує список команд трасування променів, створених за допомогою методів ``raytracing_*``.
...
Встановлює постійні дані, що передаються, у ``buffer`` для вказаного ``raytracing_list``. Шейдер визначає, як ці двійкові дані будуть використовуватися. Розмір буфера в байтах також необхідно вказати в ``size_bytes`` (його можна отримати, викликавши метод :ref:`PackedByteArray.size()<class_PackedByteArray_method_size>` для переданого ``buffer``).
...
Ініціалізує диспетчеризацію трасування променів для ``raytracing_list``, запускаючи ``width`` × ``height`` × ``depth`` променів.

\ ``raygen_shader_index`` вибирає шейдер генерації променів з конвеєра, прив'язаного за допомогою :ref:`raytracing_list_bind_raytracing_pipeline()<class_RenderingDevice_method_raytracing_list_bind_raytracing_pipeline>`.

\ ``hit_sbt`` повинен використовувати той самий конвеєр, що й ``raytracing_list``.
...
Створює новий конвеєр трасування променів. Доступ до нього можна отримати за допомогою повернутого ідентифікатора RID.

Після завершення роботи з RID слід звільнити його за допомогою методу :ref:`free_rid()<class_RenderingDevice_method_free_rid>` класу RenderingDevice.

Кожен шейдер повинен надавати необхідний етап. Усі етапи повинні використовувати сумісні схеми конвеєра. Конвеєр вибирає необхідний етап з кожного шейдера.

Порядок введення визначає стабільні індекси, що використовуються API:

- ``raygen_shaders`` індексується в :ref:`raytracing_list_trace_rays()<class_RenderingDevice_method_raytracing_list_trace_rays>`.

- ``miss_shaders`` індексується в ``traceRayEXT``.

- ``hit_groups`` індексується в :ref:`hit_sbt_range_update()<class_RenderingDevice_method_hit_sbt_range_update>`.
...
Повертає ``true``, якщо конвеєр трасування променів, вказаний за допомогою RID ``raytracing_pipeline``, є дійсним; в іншому випадку — ``false``.
...
Копіює ``from_texture`` у ``to_texture`` із зазначеними координатами ``from_pos``, ``to_pos`` та ``size``. Для двовимірних текстур ``from_pos`` та ``to_pos`` повинні мати вісь Z з значенням ``0``, а ``size`` — вісь Z з значенням ``1``. Також необхідно вказати вихідні та цільові міп-мапи/шари, причому ці параметри дорівнюють ``0`` для текстур без міп-мап або одношарових текстур. Повертає :ref:`@GlobalScope.OK<class_@GlobalScope_constant_OK>`, якщо копіювання текстури пройшло успішно, або :ref:`@GlobalScope.ERR_INVALID_PARAMETER<class_@GlobalScope_constant_ERR_INVALID_PARAMETER>` в іншому випадку.

\ **Примітка:** Текстуру ``from_texture`` не можна скопіювати, поки створюється список малювання, який використовує її як частину фреймбуфера. Переконайтеся, що список малювання завершено (і що текстура кольору/глибини, яка його використовує, не встановлена на :ref:`FINAL_ACTION_CONTINUE<class_RenderingDevice_constant_FINAL_ACTION_CONTINUE>`), щоб скопіювати цю текстуру.

\ **Примітка:** Текстура ``from_texture`` вимагає, щоб було отримано :ref:`TEXTURE_USAGE_CAN_COPY_FROM_BIT<class_RenderingDevice_constant_TEXTURE_USAGE_CAN_COPY_FROM_BIT>`.

\ **Примітка:** ``to_texture`` не можна скопіювати, поки створюється список малювання, який використовує її як частину фреймбуфера. Переконайтеся, що список малювання завершено (і що текстура кольору/глибини, яка її використовує, не встановлена на :ref:`FINAL_ACTION_CONTINUE<class_RenderingDevice_constant_FINAL_ACTION_CONTINUE>`), щоб скопіювати цю текстуру.

\ **Примітка:** ``to_texture`` вимагає отримання ``константи TEXTURE_USAGE_CAN_COPY_TO_BIT``.

\ **Примітка:** ``from_texture`` та ``to_texture`` повинні бути одного типу (колір або глибина).
...
Створює ``tlas``. Вміст попередніх збірок видаляється.

Будь-які BLAS, надані через елемент :ref:`RDAccelerationStructureInstance.blas<class_RDAccelerationStructureInstance_property_blas>`, повинні бути попередньо створені за допомогою методу :ref:`blas_build()<class_RenderingDevice_method_blas_build>`.

Кількість екземплярів може дорівнювати або бути меншою за максимальну кількість екземплярів, передбачену в методі :ref:`tlas_create()<class_RenderingDevice_method_tlas_create>`.

\ **Примітка:** Звільнення або перекомпіляція будь-якого з наданих BLAS після цього методу робить TLAS недійсним і вимагає його перекомпіляції.
...
Створює нову структуру прискорення верхнього рівня (TLAS). Доступ до неї можна отримати за допомогою повернутого ідентифікатора RID.
