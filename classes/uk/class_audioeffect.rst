...
Базовий об’єкт :ref:`Resource<class_Resource>` для кожного аудіоефекту. У редакторі аудіоефект можна додати до поточної схеми шини за допомогою панелі «Аудіо». Під час виконання також можна керувати аудіоефектами за допомогою методів ``AudioServer.add_bus_effect``, ``AudioServer.remove_bus_effect`` та ``AudioServer.get_bus_effect``.

При застосуванні до шини аудіоефект створює відповідний :ref:`AudioEffectInstance<class_AudioEffectInstance>`. Цей екземпляр безпосередньо відповідає за маніпулювання звуком на основі властивостей оригінального аудіоефекту.
