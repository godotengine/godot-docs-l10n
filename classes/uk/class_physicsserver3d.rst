...
**Успадковано від:** :ref:`PhysicsServer3DExtension<class_PhysicsServer3DExtension>`
...
:ref:`Joint3D<class_Joint3D>` є :ref:`PinJoint3D<class_PinJoint3D>`.
...
:ref:`Joint3D<class_Joint3D>` є :ref:`HingeJoint3D<class_HingeJoint3D>`.
...
:ref:`Joint3D<class_Joint3D>` є :ref:`SliderJoint3D<class_SliderJoint3D>`.
...
:ref:`Joint3D<class_Joint3D>` є :ref:`ConeTwistJoint3D<class_ConeTwistJoint3D>`.
...
:ref:`Joint3D<class_Joint3D>` є :ref:`Generic6DOFJoint3D<class_Generic6DOFJoint3D>`.
...
Представляємо розмір об'єкта :ref:`JointType<enum_PhysicsServer3D_JointType>`.
...
Сила, з якою закріплені об’єкти намагаються зберігати взаємне положення. Чим вище значення, тим сильніше.

\ **Примітка:** Підтримується лише при використанні GodotPhysics3D. Цей параметр ігнорується при використанні Jolt Physics.
...
Сила, з якою закріплені об’єкти намагаються зберігати відносну швидкість один щодо одного. Чим вище значення, тим сильніше.
...
Якщо значення більше 0, це максимальне значення імпульсу, яке цей Joint3D може передавати на свої кінці.
...
Швидкість, з якою два тіла зближуються, рухаючись у різних напрямках.
...
Швидкість, з якою коригується обертання навколо осі, перпендикулярної до шарніра.
...
Чим нижче це значення, тим більше сповільнюється обертання.
...
Коефіцієнт, що застосовується до руху вздовж осі повзунка після перевищення меж. Чим менше значення, тим повільніше рух.
...
Величина відскоку після перевищення обмежень. Чим вона менша, тим більше енергії швидкості втрачається.
...
Ступінь гальмування після перевищення меж повзунка.
...
Коефіцієнт, що застосовується до руху вздовж осі повзунка, поки повзунок знаходиться в межах. Чим менше значення, тим повільніше рух.
...
Величина відновлення в межах повзунка.
...
Ступінь демпфірування в межах повзунка.
...
Коефіцієнт, що застосовується до руху вздовж осей, перпендикулярних до повзунка.
...
Величина відскоку при русі вздовж осей, перпендикулярних до повзунка.
...
Ступінь гальмування при русі вздовж осей, перпендикулярних до напрямку руху.
...
Коефіцієнт, що застосовується до всіх обертань після перевищення обмеження.
...
Величина відновлення обертання при перевищенні обмеження.
...
Ступінь гальмування обертання при перевищенні обмеження.
...
Коефіцієнт, що застосовується до загального кута повороту в межах.
...
Величина відновлення обертання в межах.
...
Ступінь гальмування обертання в межах.
...
Коефіцієнт, що застосовується до всіх обертань навколо осей, перпендикулярних до повзунка.
...
Величина відновлення обертання навколо осей, перпендикулярних до повзунка.
...
Ступінь гальмування обертання навколо осей, перпендикулярних до напрямку руху.
...
Представляємо розмір :ref:`SliderJointParam<enum_PhysicsServer3D_SliderJointParam>` enum.
...
Поворот — це обертання з боку в бік навколо осі, перпендикулярної осі скручування.

Діапазон повороту визначає, наскільки обертання не буде коригуватися вздовж осі повороту.

Цей параметр можна налаштувати в :ref:`ConeTwistJoint3D<class_ConeTwistJoint3D>`. Якщо значення менше 0,05, ця функція заблокована.
...
«Поворот» — це обертання навколо осі повороту; це значення визначає, наскільки суглоб може повертатися. Якщо значення менше 0,05, поворот блокується.
...
Швидкість, з якою відбуватиметься поворот або обертання. Чим вище значення, тим швидше.
...
Легкість, з якою обертається Joint3D: якщо значення занадто низьке, для обертання з’єднання потрібно докласти більше зусиль.
...
Визначає, як швидко синхронізується різниця в швидкості повороту та обертання з обох боків.
...
Коефіцієнт, що застосовується до руху вздовж осей. Чим менше значення, тим повільніше рух.
...
Величина відскоку при русі по осях. Чим вона менша, тим більше втрачається швидкісної енергії.
...
Ступінь демпфірування, що виникає під час лінійного руху вздовж осей.
...
Коефіцієнт, який множиться на всі обертання навколо осей.
...
Ступінь обертального демпфірування по осях. Чим нижче значення, тим сильніше демпфірування.
...
Величина відскоку при обертанні навколо осей. Чим менше значення, тим сильніший відскок.
...
Максимальне значення сили, яке може виникнути під час обертання навколо осей.
...
При корекції перевищення меж обертання навколо осей цей коефіцієнт похибки визначає, наскільки сповільнюється процес корекції. Чим нижче значення, тим повільніше.
...
Представляє розмір :ref:`G6DOFJointAxisParam<enum_PhysicsServer3D_G6DOFJointAxisParam>` enum.
...
Представляє розмір :ref:`G6DOFJointAxisFlag<enum_PhysicsServer3D_G6DOFJointAxisFlag>` enum.
...
Константа для створення форми межі світу (використовується ресурсом :ref:`WorldBoundaryShape3D<class_WorldBoundaryShape3D>`).
...
Константа для створення форми розділового променя (використовується ресурсом :ref:`SeparationRayShape3D<class_SeparationRayShape3D>`).
...
Константа для створення сферичної форми (використовується ресурсом :ref:`SphereShape3D<class_SphereShape3D>`).
...
Константа для створення форми прямокутника (використовується ресурсом :ref:`BoxShape3D<class_BoxShape3D>`).
...
Константа для створення форми капсули (використовується ресурсом :ref:`CapsuleShape3D<class_CapsuleShape3D>`).
...
Константа для створення циліндричної форми (використовується ресурсом :ref:`CylinderShape3D<class_CylinderShape3D>`).
...
Константа для створення опуклої багатокутної форми (використовується ресурсом :ref:`ConvexPolygonShape3D<class_ConvexPolygonShape3D>`).
...
Константа для створення увігнутої форми багатокутника (трисітки) (використовується ресурсом :ref:`ConcavePolygonShape3D<class_ConcavePolygonShape3D>`).
...
Константа для створення фігури карти висот (використовується ресурсом :ref:`HeightMapShape3D<class_HeightMapShape3D>`).
...
Константа, що використовується внутрішньо для м'якої форми тіла. Будь-яка спроба створити таку форму призводить до помилки.
...
Константа, що використовується внутрішньо для власної форми. Будь-яка спроба створити таку форму призводить до помилки під час використання фізики Годо або фізики поштовхів.
...
Постійно / встановити режим перевитрату ваги в зоні. Див. :ref:`AreaSpaceOverrideMode<enum_PhysicsServer3D_AreaSpaceOverrideMode>` для можливих значень.
...
Постійно встановити / вийняти лінійний демпферний режим перенади в зоні. Див. :ref:`AreaSpaceOverrideMode<enum_PhysicsServer3D_AreaSpaceOverrideMode>` для можливих значень.
...
Постійно встановити / вийняти кутовий демпферний режим перенади в зоні. Див. :ref:`AreaSpaceOverrideMode<enum_PhysicsServer3D_AreaSpaceOverrideMode>` для можливих значень.
...
Постійне встановлення/витратити величину об’ємної вітрової сили. Ця сила вітру застосовується тільки до вузлів :ref:`SoftBody3D<class_SoftBody3D>`. Інші фізико-фізичні органи в даний час не впливають на вітер.
...
Постійно встановити / вийняти лінійний режим демппінгу тіла. Див. :ref:`BodyDampMode<enum_PhysicsServer3D_BodyDampMode>` для можливих значень.
...
Постійно встановити / вийняти кутовий режим видалення тіла. Див. :ref:`BodyDampMode<enum_PhysicsServer3D_BodyDampMode>` для можливих значень.
...
Представляємо розмір :ref:`BodyParameter<enum_PhysicsServer3D_BodyParameter>` enum.
...
Константа для встановлення/отримання порогового значення лінійної швидкості активності. Об’єкт, позначений як потенційно неактивний як за лінійною, так і за кутовою швидкістю, буде переведений у режим очікування через вказаний проміжок часу.
...
Константа для встановлення/отримання порогового значення кутової швидкості активності. Об’єкт, позначений як потенційно неактивний як за лінійною, так і за кутовою швидкістю, буде переведений у режим очікування через вказаний проміжок часу.
...
Призначає область нащадку :ref:`Object<class_Object>`, щоб вона могла існувати у дереві вузлів.
...
Створює об'єкт площею 3D у фізичному сервері, і повертає :ref:`RID<class_RID>`, який ідентифікує його. Параметри за замовчуванням для створеної площі включають шар зіткнення і набір масок для ``1``, і ``моніторинг``, встановлений до ``false``.

Використовуйте :ref:`area_add_shape()<class_PhysicsServer3D_method_area_add_shape>`, щоб додати форми до нього, скористайтеся :ref:`area_set_transform()<class_PhysicsServer3D_method_area_set_transform>`, щоб встановити його трансформацію, а також використовувати :ref:`area_set_space()<class_PhysicsServer3D_method_area_set_space>` для додавання площі до простору. Якщо ви хочете, щоб зона була виявлена :ref:`area_set_monitorable()<class_PhysicsServer3D_method_area_set_monitorable>`.
...
Повертає значення параметра області. Перелік доступних параметрів знаходиться на території :ref:`AreaParameter<enum_PhysicsServer3D_AreaParameter>`.
...
Повернення :ref:`RID<class_RID>` n-ї форми ділянки.
...
1. ціле ``статус``: або :ref:`AREA_BODY_ADDED<class_PhysicsServer3D_constant_AREA_BODY_ADDED>` або :ref:`AREA_BODY_REMOVED<class_PhysicsServer3D_constant_AREA_BODY_REMOVED>` в залежності від того, чи вводилася форма іншої області або виходу на область,
...
Встановлює значення параметра області. Перелік доступних параметрів знаходиться на території :ref:`AreaParameter<enum_PhysicsServer3D_AreaParameter>`.
...
Встановлює задану форму ділянки іншим. Стара форма вибирається за індексом, новий за його :ref:`RID<class_RID>`.
...
Це еквівалентно використання :ref:`body_add_constant_force()<class_PhysicsServer3D_method_body_add_constant_force>` в центрі маси тіла.
...
Це еквівалентно використанню :ref:`body_apply_force()<class_PhysicsServer3D_method_body_apply_force>` в центрі маси тіла.
...
Це еквівалентно використанню :ref:`body_apply_impulse()<class_PhysicsServer3D_method_body_apply_impulse>` в центрі маси тіла.
...
Створює об'єкт 3D в фізичному сервері, і повертає :ref:`RID<class_RID>`, який ідентифікує його. Параметри за замовчуванням для створеної площі включають шар зіткнення і набір масок для ``1``, і режим кузова, встановленого до :ref:`BODY_MODE_RIGID<class_PhysicsServer3D_constant_BODY_MODE_RIGID>`.

Використовуйте :ref:`body_add_shape()<class_PhysicsServer3D_method_body_add_shape>`, щоб додати форми до нього, скористайтеся :ref:`body_set_state()<class_PhysicsServer3D_method_body_set_state>`, щоб встановити його трансформацію, і використовувати :ref:`body_set_space()<class_PhysicsServer3D_method_body_set_space>`, щоб додати тіло до простору.
...
Див. :ref:`body_add_constant_force()<class_PhysicsServer3D_method_body_add_constant_force>` та :ref:`body_add_constant_central_force()<class_PhysicsServer3D_method_body_add_constant_central_force>`.
...
Подивитися :ref:`body_add_constant_torque()<class_PhysicsServer3D_method_body_add_constant_torque>`.
...
Повертає :ref:`PhysicsDirectBodyState3D<class_PhysicsDirectBodyState3D>` тіла. Повертає ``null``, якщо тіло знищено або видалено з фізичного простору.
...
Повернутися до максимальних контактів, які можуть бути повідомлені. Див. :ref:`body_set_max_contacts_reported()<class_PhysicsServer3D_method_body_set_max_contacts_reported>`.
...
Повертає значення параметра тіла. Перелік доступних параметрів знаходиться на константі :ref:`BodyParameter<enum_PhysicsServer3D_BodyParameter>`.
...
Повернення :ref:`RID<class_RID>` n-ї форми тіла.
...
Повертаємо ``true``, якщо тіло відмовляється від стандартної силової інтеграції. Див. :ref:`_set_omit_force_integration()<class_PhysicsServer3D_private_method__set_omit_force_integration>`.
...
Відновлює значення інерції та центру мас за замовчуванням на основі форм, щоб скасувати будь-які користувацькі значення, встановлені раніше за допомогою :ref:`body_set_param()<class_PhysicsServer3D_method_body_set_param>`.
...
Функція ``callable`` буде називатися кожен фізичний кліщ, перш ніж стандартна інтеграція сили (див. :ref:`body_set_omit_force_integration()<class_PhysicsServer3D_method_body_set_omit_force_integration>`). Для оновлення лінійної та кутової швидкості тіла можна використовувати наприклад, для оновлення лінійної та кутової швидкості на основі контакту з іншими тілами.

Якщо ``userdata`` не ``null``, функція ``callable`` повинна приймати наступні два параметри:

1. ``state``: a :ref:`PhysicsDirectBodyState3D<class_PhysicsDirectBodyState3D>`, використовується для отримання та модифікації стану тіла,

2. ``userdata``: a :ref:`Variant<class_Variant>`; його значення буде ``userdata`` передано в цей метод.
...
Цей метод називається при наявності майна :ref:`RigidBody3D.custom_integrator<class_RigidBody3D_property_custom_integrator>`.
...
Налаштовує параметр тіла. Перелік доступних параметрів знаходиться на константі :ref:`BodyParameter<enum_PhysicsServer3D_BodyParameter>`.
...
Налаштовує задану форму тіла іншим. Стара форма вибирається за індексом, новий за його :ref:`RID<class_RID>`.
...
Призначає пробіл тілу (див. :ref:`space_create()<class_PhysicsServer3D_method_space_create>`).
...
Налаштовує функцію зворотного виклику стану тіла до ``callable``. Використовуйте порожній :ref:`Callable<class_Callable>` (``Callable()``) для очищення зворотного виклику.
...
1. ``state``: a :ref:`PhysicsDirectBodyState3D<class_PhysicsDirectBodyState3D>`, використовується для отримання стану тіла.
...
Повертаємо ``true``, якщо зіткнення призведе до переміщення по вектору руху з даної точки в космосі. :ref:`PhysicsTestMotionParameters3D<class_PhysicsTestMotionParameters3D>` передається для встановлення параметрів руху. :ref:`PhysicsTestMotionResult3D<class_PhysicsTestMotionResult3D>` може передати додаткову інформацію.
...
Створює 3D-форму прямокутника на сервері фізики та повертає :ref:`RID<class_RID>`, який його ідентифікує. Використовуйте :ref:`shape_set_data()<class_PhysicsServer3D_method_shape_set_data>` для встановлення половинних розмірів прямокутника.
...
Створює 3D-форму капсули на сервері фізики та повертає :ref:`RID<class_RID>`, який її ідентифікує. Використовуйте :ref:`shape_set_data()<class_PhysicsServer3D_method_shape_set_data>` для встановлення висоти та радіуса капсули.
...
Створює тривимірну увігнуту фігуру багатокутника на сервері фізики та повертає :ref:`RID<class_RID>`, який її ідентифікує. Використовуйте метод :ref:`shape_set_data()<class_PhysicsServer3D_method_shape_set_data>` для встановлення трикутників увігнутого багатокутника.
...
Створює опуклий тривимірний багатокутник на сервері фізики та повертає :ref:`RID<class_RID>`, який його ідентифікує. Використовуйте метод :ref:`shape_set_data()<class_PhysicsServer3D_method_shape_set_data>` для встановлення точок опуклого багатокутника.
...
Створює власну фігуру на сервері фізики та повертає :ref:`RID<class_RID>`, який її ідентифікує. Використовуйте :ref:`shape_set_data()<class_PhysicsServer3D_method_shape_set_data>` для встановлення даних фігури.

\ **Примітка:** Вбудовані сервери фізики не підтримують власні фігури, тому виклик цього методу завжди призводить до помилки під час використання Godot Physics або Jolt Physics. Власні сервери фізики, реалізовані як GDExtensions, можуть підтримувати власну фігуру.
...
Створює 3D-форму циліндра на сервері фізики та повертає :ref:`RID<class_RID>`, який її ідентифікує. Використовуйте :ref:`shape_set_data()<class_PhysicsServer3D_method_shape_set_data>` для встановлення висоти та радіуса циліндра.
...
Дестройс будь-який з об'єктів, створених фізикоюServer3D. Якщо :ref:`RID<class_RID>` було передано не одне з об'єктів, які можуть бути створені за допомогою PhysServer3D, похибка буде відправлена до консолі.
...
Створює 3D-фігуру карти висот на сервері фізики та повертає :ref:`RID<class_RID>`, який її ідентифікує. Використовуйте :ref:`shape_set_data()<class_PhysicsServer3D_method_shape_set_data>` для встановлення даних карти висот.
...
Налаштовує, чи кріпляться ті тіла до :ref:`Joint3D<class_Joint3D>`.
...
Отримує значення пріоритету Joint3D.

\ **Примітка:** Підтримується лише при використанні GodotPhysics3D. При використанні Jolt Physics цей метод завжди повертає ``1``, оскільки Jolt Physics не підтримує пріоритет обчислювача з'єднань.
...
Повертаємо вашу увагу на те, що ті органи, прикріплені до :ref:`Joint3D<class_Joint3D>`, зіштовхуються один з одним.
...
Зробіть з'єднання загальним з'єднанням із шістьма ступенями свободи (6DOF). Використовуйте метод :ref:`generic_6dof_joint_set_flag()<class_PhysicsServer3D_method_generic_6dof_joint_set_flag>` та :ref:`generic_6dof_joint_set_param()<class_PhysicsServer3D_method_generic_6dof_joint_set_param>` для встановлення прапорців та параметрів з'єднання відповідно.
...
Встановлює значення пріоритету Joint3D.

\ **Примітка:** Підтримується лише при використанні GodotPhysics3D. Цей метод не діє при використанні Jolt Physics, оскільки ця бібліотека не підтримує пріоритет обчислювача з’єднань.
...
Створює 3D-фігуру розділового променя на сервері фізики та повертає :ref:`RID<class_RID>`, який її ідентифікує. Використовуйте :ref:`shape_set_data()<class_PhysicsServer3D_method_shape_set_data>` для встановлення властивостей фігури ``length`` та ``slide_on_slope``.
...
Повертає дані фігури, які її налаштовують, наприклад, половинні протяжності прямокутника або трикутники увігнутої (трисітчастої) фігури. Див. :ref:`shape_set_data()<class_PhysicsServer3D_method_shape_set_data>` для точного формату цих даних у кожному випадку.
...
Встановлює дані фігури, які налаштовують її. Дані ``data``, що передаються, залежать від типу фігури (див. :ref:`shape_get_type()<class_PhysicsServer3D_method_shape_get_type>`):

- :ref:`SHAPE_WORLD_BOUNDARY<class_PhysicsServer3D_constant_SHAPE_WORLD_BOUNDARY>`: :ref:`Plane<class_Plane>`,

- :ref:`SHAPE_SEPARATION_RAY<class_PhysicsServer3D_constant_SHAPE_SEPARATION_RAY>`: словник, що містить ключ ``"length"`` зі значенням :ref:`float<class_float>` та ключ ``"slide_on_slope"`` зі значенням :ref:`bool<class_bool>`,

- :ref:`SHAPE_SPHERE<class_PhysicsServer3D_constant_SHAPE_SPHERE>`: :ref:`float<class_float>`, що є радіусом сфери,

- :ref:`SHAPE_BOX<class_PhysicsServer3D_constant_SHAPE_BOX>`: :ref:`Vector3<class_Vector3>`, що містить половини об'єму прямокутника,

- :ref:`SHAPE_CAPSULE<class_PhysicsServer3D_constant_SHAPE_CAPSULE>`: словник, що містить ключі ``"height"`` та ``"radius"`` зі значеннями :ref:`float<class_float>`,

- :ref:`SHAPE_CYLINDER<class_PhysicsServer3D_constant_SHAPE_CYLINDER>`: словник, що містить ключі ``"height"`` та ``"radius"`` зі значеннями :ref:`float<class_float>`,

- :ref:`SHAPE_CONVEX_POLYGON<class_PhysicsServer3D_constant_SHAPE_CONVEX_POLYGON>`: масив точок :ref:`PackedVector3Array<class_PackedVector3Array>`, що визначають опуклий багатокутник (форма буде опуклою оболонкою точок),

- :ref:`SHAPE_CONCAVE_POLYGON<class_PhysicsServer3D_constant_SHAPE_CONCAVE_POLYGON>`: словник, що містить ключ ``"faces"`` зі значенням :ref:`PackedVector3Array<class_PackedVector3Array>` (з довжиною, що ділиться на 3, так що кожен кортеж із 3 точок утворює грань) та ключ ``"backface_collision"`` зі значенням :ref:`bool<class_bool>`,

- :ref:`SHAPE_HEIGHTMAP<class_PhysicsServer3D_constant_SHAPE_HEIGHTMAP>`: словник, що містить ключі ``"width"`` та ``"depth"`` зі значеннями :ref:`int<class_int>`, та ключ ``"heights"`` зі значенням, яке є упакованим масивом :ref:`float<class_float>`\ s довжиною ``width * depth`` (тобто :ref:`PackedFloat32Array<class_PackedFloat32Array>` або :ref:`PackedFloat64Array<class_PackedFloat64Array>`, якщо Godot було скомпільовано з опцією ``precision=double``), та опціонально ключі ``"min_height"`` та ``"max_height"`` зі значеннями :ref:`float<class_float>`,

- :ref:`SHAPE_SOFT_BODY<class_PhysicsServer3D_constant_SHAPE_SOFT_BODY>`: вхідні дані ``data`` ігноруються, і цей метод не має жодного ефекту,

- :ref:`SHAPE_CUSTOM<class_PhysicsServer3D_constant_SHAPE_CUSTOM>`: вхідні дані ``data`` інтерпретуються користувацьким сервером фізики, якщо він підтримує користувацькі форми.
...
Створює новий м'який корпус і повертає його внутрішню :ref:`RID<class_RID>`.
...
Повертаємо :ref:`RID<class_RID>` простору, призначеного для даного м'якого тіла.
...
\ **Примітка:** Натискання точки ефективно робить його кінематичними, запобігаючи її впливу на сили, але ви все ще можете переміщати його за допомогою :ref:`Soft_body_move_point()<class_PhysicsServer3D_method_Soft_body_move_point>`.
...
Призначає простір заданому м’якому тілу (див. :ref:`space_create()<class_PhysicsServer3D_method_space_create>`).
...
Створення простору. Простір - це збір параметрів для фізичного двигуна, який може бути призначений для зони або тіла. Ви можете призначати область з :ref:`area_set_space()<class_PhysicsServer3D_method_area_set_space>`, або до тіла з :ref:`body_set_space()<class_PhysicsServer3D_method_body_set_space>`.
...
Повертає стан простору, :ref:`PhysicsDirectSpaceState3D<class_PhysicsDirectSpaceState3D>`. Цей об'єкт можна використовувати для створення запитів на колізії/перетини.
...
Встановлює значення для параметра простору. Перелік доступних параметрів знаходиться на константі :ref:`SpaceParameter<enum_PhysicsServer3D_SpaceParameter>`.
...
Створює 3D-сферу на сервері фізики та повертає :ref:`RID<class_RID>`, який її ідентифікує. Використовуйте :ref:`shape_set_data()<class_PhysicsServer3D_method_shape_set_data>` для встановлення радіуса сфери.
...
Створює фігуру межі 3D-світу на сервері фізики та повертає :ref:`RID<class_RID>`, який її ідентифікує. Використовуйте метод :ref:`shape_set_data()<class_PhysicsServer3D_method_shape_set_data>` для встановлення властивостей напрямку нормалі та відстані фігури.
