...
Масив, спеціально призначений для зберігання 64-бітних цілих чисел. Забезпечує щільне упакування даних, що дозволяє економити пам'ять при роботі з великими масивами.

\ **Примітка:** Цей тип зберігає 64-бітні цілі числа зі знаком, що означає, що він може приймати значення в діапазоні ``[-2^63, 2^63 - 1]``, тобто `` [-9223372036854775808, 9223372036854775807]``. При перевищенні цих меж значення переносяться. Якщо вам потрібно щільно упакувати лише 32-бітні цілі числа, дивіться :ref:`PackedInt32Array<class_PackedInt32Array>` для альтернативи, що менше споживає пам'яті.

\ **Відмінності між упакованими масивами, типізованими масивами та нетипізованими масивами:** Упаковані масиви, як правило, швидше проходять ітерацію та модифікуються порівняно з типізованим масивом того ж типу (наприклад, **PackedInt64Array** проти ``Array[int]``). Крім того, упаковані масиви споживають менше пам'яті. З іншого боку, упаковані масиви менш гнучкі, оскільки не пропонують стільки зручних методів, як, наприклад, :ref:`Array.map()<class_Array_method_map>`. Типізовані масиви, у свою чергу, швидше обробляються та модифікуються, ніж нетипізовані масиви.
