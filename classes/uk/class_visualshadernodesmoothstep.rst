...
Розрахунок функції SmoothStep в графі візуального відтінку.
...
Перетворюється на ``smoothstep(edge0, edge1, x)`` мовою shar.

\ ``0.0``, якщо ``x`` менше ``edge0`` і ``1.0``, якщо ``x`` більше, ніж ``edge1``. В іншому випадку значення повернення між собою ``0.0`` і ``1.0`` за допомогою поліномій Hermite.
...
``x`` порт використовує 2D векторний тип. Перші два порти використовують плавучий тип скаляра.
...
``x`` порт використовує 3D векторний тип. Перші два порти використовують плавучий тип скаляра.
...
Представляємо розмір :ref:`OpType<enum_VisualShaderNodeSmoothStep_OpType>` enum.
