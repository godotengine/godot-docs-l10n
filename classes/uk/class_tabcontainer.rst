...
**Застаріло:** Due to internal changes this doesn't do anything anymore, as they're always in front.
...
Якщо ``true``, вкладка з індексом ``index`` вимкнена.

\ **Примітка:** ``index`` — це значення в діапазоні ``0 .. get_tab_count() - 1``.
...
Якщо ``true``, вкладка з індексом ``index`` прихована.
...
Текст заголовка вкладки на ``index``.
...
Текст підказки для вкладки з індексом ``index``.
...
Піктограма, що вказує місце, куди буде переміщено перетягнуту вкладку (див. :ref:`drag_to_rearrange_enabled<class_TabContainer_property_drag_to_rearrange_enabled>`).
