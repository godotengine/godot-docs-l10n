...
**SpinBox** — це текстове поле для введення чисел. Воно дозволяє вводити цілі та дійсні числа. **SpinBox** також має кнопки «вгору» та «вниз», натискання яких дозволяє збільшувати або зменшувати значення. Значення також можна змінити, перетягнувши курсор миші вгору або вниз по стрілках **SpinBox**.

Крім того, можна вводити математичні вирази. Вони обчислюються, коли користувач натискає :kbd:`Enter` під час редагування текстового поля **SpinBox**. Для розбору та обчислення виразу використовується клас :ref:`Expression<class_Expression>`. Результат виразу потім встановлюється як значення **SpinBox**. Деякі приклади допустимих виразів: ``5 + 2 * 3``, ``pow(2, 4)`` та ``PI + sin(0.5)``. У виразах враховується регістр.
...
Дивіться клас :ref:`Range<class_Range>` для додаткових опцій щодо **SpinBox**.

\ **Примітка:** Якщо контекстне меню **SpinBox** вимкнено, ви можете клацнути правою кнопкою миші на нижній половині спінбокса, щоб встановити мінімальне значення, а клацнувши правою кнопкою миші на верхній половині — максимальне.

\ **Примітка:** **SpinBox** базується на базовому вузлі :ref:`LineEdit<class_LineEdit>`. Щоб змінити тему фону **SpinBox**, додайте елементи теми для :ref:`LineEdit<class_LineEdit>` та налаштуйте їх. :ref:`LineEdit<class_LineEdit>` має варіант теми ``SpinBoxInnerLineEdit``, завдяки чому ви можете надати йому вигляд, відмінний від звичайних :ref:`LineEdit<class_LineEdit>`.

\ **Примітка:** Якщо ви хочете реалізувати перетягування для базового :ref:`LineEdit<class_LineEdit>`, ви можете використовувати :ref:`Control.set_drag_forwarding()<class_Control_method_set_drag_forwarding>` на вузлі, поверненому :ref:`get_line_edit()<class_SpinBox_method_get_line_edit>`.
