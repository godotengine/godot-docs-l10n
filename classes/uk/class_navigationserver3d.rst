...
НавігаціяServer3D - сервер, який керує навігацією, регіонами та агентами. Навігація A\* від :ref:`AStar3D<class_AStar3D>`.
...
Ви можете призначити навігаційні шари до регіонів з :ref:`region_set_navigation_layers()<class_NavigationServer3D_method_region_set_navigation_layers>`, які потім можна перевірити при запитуванні шляху з :ref:`map_get_path()<class_NavigationServer3D_method_map_get_path>`. Це може бути використаний для того, щоб дозволити або заперечувати певні ділянки для деяких об'єктів.
...
Набори ``velocity`` як нова захожена швидкість за вказаною ``agent``. Симулятор уникнення зіткнень з іншими перешкодами агента. Коли агент зв'язується з новим використанням позиції :ref:`agent_set_velocity_forced()<class_NavigationServer3D_method_agent_set_velocity_forced>`, а також для скидання внутрішньої швидкості моделювання.
...
Навігаційні шари посилань. Це дозволяє вибрати посилання з запиту на шлях (при використанні :ref:`map_get_path()<class_NavigationServer3D_method_map_get_path>`).
...
Парус :ref:`SceneTree<class_SceneTree>` для геометрії джерела за властивостями ``Navigation_mesh``. Оновлення наданої ``джерело_geometry_data`` ресурсу з отриманими даними. Після цього ресурс може бути використаний для запікання навігаційної сітки з :ref:`bake_from_source_geometry_data()<class_NavigationServer3D_method_bake_from_source_geometry_data>`. Після завершення процесу буде викликано додаткове ``callback``.
...
Запитує шлях на даній навігаційній карті. Початкова та цільова позиція та інші параметри визначаються через :ref:`NavigationPathQueryParameters3D<class_NavigationPathQueryParameters3D>`. Оновлює наданий об’єкт результату :ref:`NavigationPathQueryResult3D<class_NavigationPathQueryResult3D>` за допомогою шляху серед інших результатів, запитуваних запитом. Після завершення процесу буде викликано додатковий ``callback``.
...
Повертає кінцеву точку з'єднання door. ``connection`` – це індекс між 0 та поверненим значенням :ref:`region_get_connections_count()<class_NavigationServer3D_method_region_get_connections_count>`.
...
Повертає початкову точку з'єднання door. ``connection`` – це індекс між 0 та значенням, що повертається методом :ref:`region_get_connections_count()<class_NavigationServer3D_method_region_get_connections_count>`.
...
Встановіть навігаційні шари регіону. Це дозволяє вибрати регіони з запиту на шлях (при використанні :ref:`map_get_path()<class_NavigationServer3D_method_map_get_path>`).
...
Створення нового генератора геометрії джерела. Якщо :ref:`Callable<class_Callable>` встановлюється для парсера з :ref:`source_geometry_parser_set_callback()<class_NavigationServer3D_method_source_geometry_parser_set_callback>`, зворотний дзвінок буде викликано для кожного окремого вузла, який отримує парсе, коли methodметод parse_source_geometry_data].
...
- ``navigation_mesh`` - Довідник :ref:`NavigationMesh<class_NavigationMesh>` використовується для визначення параметрів парсера. Не відредагуйте або додайте безпосередньо до навігаційної сітки.

- ``source_geometry_data`` - ``НавігаціяMeshSourceGeometryData3D`` посилання. Додати геометрію джерела для навігаційної сітки для цього об'єкта.

- ``node`` - :ref:`Node<class_Node>`, який знаходиться в парсі.
