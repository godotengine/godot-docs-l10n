...
Лінійний інтерполює між двома значеннями в графі візуального відтінку.
...
Перекладається як ``mix(a, b, weight)`` мовою shar.
...
``a`` і ``b`` порти використовують 2D векторний тип. `` вага`` порт використовує тип скаляра.
...
``a`` і ``b`` порти використовують 3D векторний тип. `` вага`` порт використовує тип скаляра.
...
Представляємо розмір :ref:`OpType<enum_VisualShaderNodeMix_OpType>` enum.
