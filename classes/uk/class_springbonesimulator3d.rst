...
Цей :ref:`SkeletonModifier3D<class_SkeletonModifier3D>` можна використовувати для імітації коливання волосся, одягу та хвостів. Цей модифікатор працює інакше, ніж :ref:`PhysicalBoneSimulator3D<class_PhysicalBoneSimulator3D>`, оскільки він намагається повернути вихідну позу після модифікації.

Якщо ви налаштуєте :ref:`set_root_bone()<class_SpringBoneSimulator3D_method_set_root_bone>` та :ref:`set_end_bone()<class_SpringBoneSimulator3D_method_set_end_bone>`, це розглядається як один ланцюг кісток. Зверніть увагу, що він не підтримує розгалужені ланцюги, такі як Y-подібні ланцюги.

Коли створюється ланцюг кісток, з кісток, що існують між ними та перелічені у списку суглобів, генерується масив.

До кожного з'єднання можна застосувати кілька властивостей, таких як :ref:`set_joint_stiffness()<class_SpringBoneSimulator3D_method_set_joint_stiffness>`, :ref:`set_joint_drag()<class_SpringBoneSimulator3D_method_set_joint_drag>` та :ref:`set_joint_gravity()<class_SpringBoneSimulator3D_method_set_joint_gravity>`.

Для спрощення ви можете встановити значення для всіх з'єднань одночасно, використовуючи :ref:`Curve<class_Curve>`. Якщо ви хочете вказати детальні значення індивідуально, встановіть :ref:`set_individual_config()<class_SpringBoneSimulator3D_method_set_individual_config>` на ``true``.

Для фізичного моделювання **SpringBoneSimulator3D** може мати дочірні елементи як самостійні зіткнення, що не пов'язані з :ref:`PhysicsServer3D<class_PhysicsServer3D>`, див. також :ref:`SpringBoneCollision3D<class_SpringBoneCollision3D>`.

\ **Попередження:** Масштабований **SpringBoneSimulator3D**, ймовірно, не буде поводитися як очікується. Переконайтеся, що батьківський :ref:`Skeleton3D<class_Skeleton3D>` та його кістки не масштабовані.
...
Встановлює вісь обертання в ``joint`` у списку суглобів ланцюжка кісток, якщо :ref:`is_config_individual()<class_SpringBoneSimulator3D_method_is_config_individual>` має значення ``true``.

Осі базуються на просторі еталонної пози; якщо ``axis`` має значення :ref:`SkeletonModifier3D.ROTATION_AXIS_CUSTOM<class_SkeletonModifier3D_constant_ROTATION_AXIS_CUSTOM>`, ви можете вказати будь-яку вісь.

Тут опорною позою є поза кісток безпосередньо перед симуляцією.

\ **Примітка:** Вісь обертання та вектор руху вперед не повинні бути колінеарними, щоб уникнути небажаного обертання, оскільки **SpringBoneSimulator3D** не враховує сили скручування.
...
Встановлює вісь обертання ланцюжка кісток. Якщо вказано конкретну вісь, вона діє як шарнірний суглоб. Значення зберігається у кеші для кожного суглоба у списку суглобів.

Осі базуються на просторі еталонної пози; якщо ``axis`` має значення :ref:`SkeletonModifier3D.ROTATION_AXIS_CUSTOM<class_SkeletonModifier3D_constant_ROTATION_AXIS_CUSTOM>`, ви можете вказати будь-яку вісь.

Тут опорною позою є поза кісток безпосередньо перед симуляцією.

\ **Примітка:** Вектор осі обертання та вектор руху вперед не повинні бути колінеарними, щоб уникнути небажаного обертання, оскільки **SpringBoneSimulator3D** не враховує сили скручування.
