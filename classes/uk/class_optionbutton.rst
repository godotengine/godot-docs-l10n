...
Повідомлення надсилається, коли користувач переходить до елемента за допомогою дій введення :ref:`ProjectSettings.input/ui_up<class_ProjectSettings_property_input/ui_up>` або :ref:`ProjectSettings.input/ui_down<class_ProjectSettings_property_input/ui_down>`. Індекс елемента, на якому встановлено фокус, передається як аргумент.
...
Якщо ``true``, у верхній частині :ref:`PopupMenu<class_PopupMenu>` відображається рядок пошуку для фільтрування елементів. Див. :ref:`search_bar_min_item_count<class_OptionButton_property_search_bar_min_item_count>` для динамічного керування його видимістю залежно від кількості елементів.
...
Якщо ``true``, увімкне неточний пошук у :ref:`PopupMenu<class_PopupMenu>` рядку пошуку. Це дозволяє включати до результатів пошуку елементи, які майже відповідають пошуковому запиту, а також елементи, що містять окремі символи пошукового запиту, але не в тому ж порядку.

Використовуйте :ref:`search_bar_fuzzy_search_max_misses<class_OptionButton_property_search_bar_fuzzy_search_max_misses>`, щоб встановити максимальну кількість допустимих невідповідностей у результатах пошуку.
...
Встановлює максимальну кількість розбіжностей, дозволених у кожному результаті пошуку, якщо для панелі пошуку :ref:`PopupMenu<class_PopupMenu>` увімкнено неточний пошук. Усі елементи, що містять більше розбіжностей, будуть приховані в результатах пошуку.
...
Встановлює мінімальну кількість елементів, необхідну для відображення панелі пошуку :ref:`PopupMenu<class_PopupMenu>`. Щоб це спрацювало, значення :ref:`search_bar_enabled<class_OptionButton_property_search_bar_enabled>` має бути ``true``.
