...
Повертає тип конфігурації подання, який є типом `XrViewConfigurationType <https://registry.khronos.org/OpenXR/specs/1.0/man/html/XrViewConfigurationType.html>`__, перетвореним у ціле число.
...
Повертає кількість оглядів. Зазвичай їх дві — по одній для кожного ока, але ця кількість може відрізнятися залежно від конфігурації огляду.
...
Реєструє вказане розширення як таке, що змінює ``XrCompositionLayerProjection`` за допомогою віртуального методу :ref:`OpenXRExtensionWrapper._set_projection_layer_and_get_next_pointer()<class_OpenXRExtensionWrapper_private_method__set_projection_layer_and_get_next_pointer>`.

\ **Примітка:** Цю функцію не можна викликати після запуску сеансу OpenXR. Однак її можна викликати в :ref:`OpenXRExtensionWrapper._on_session_created()<class_OpenXRExtensionWrapper_private_method__on_session_created>`.
...
Скасовує реєстрацію вказаного розширення як такого, що модифікує ``XrCompositionLayerProjection``.

\ **Примітка:** Цю функцію не можна викликати, поки сесія OpenXR ще працює.
