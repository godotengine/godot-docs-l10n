...
Додає додаткові маркери перетягування на перетині маркерів двох SplitContainers, щоб можна було перетягувати обидва одночасно. Це потрібно встановити на ``true`` для обох SplitContainers, і один з них повинен бути нащадком іншого. Вони також повинні бути ортогональними (їхні :ref:`vertical<class_SplitContainer_property_vertical>` відрізняються), а нащадок повинен знаходитися поруч принаймні з одним з перетягувачів предка (в межах :ref:`minimum_grab_thickness<class_SplitContainer_theme_constant_minimum_grab_thickness>`).
