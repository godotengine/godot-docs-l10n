...
Створює новий якір, який буде відстежуватися середовищем виконання XR. ``transform`` має бути перетворенням у локальному просторі вашого вузла :ref:`XROrigin3D<class_XROrigin3D>`. Якщо ``spatial_context`` не вказано, буде використано значення за замовчуванням, для цього потрібно встановити :ref:`ProjectSettings.xr/openxr/extensions/spatial_entity/enable_builtin_anchor_detection<class_ProjectSettings_property_xr/openxr/extensions/spatial_entity/enable_builtin_anchor_detection>`. Повернений трекер відстежуватиме розташування у випадку зміни нашого простору посилань.
