...
Працює так само, як :ref:`SCROLL_MODE_AUTO<class_ScrollContainer_constant_SCROLL_MODE_AUTO>`, але змушує **ScrollContainer** повідомляти про мінімальний розмір на основі його вмісту (обмежений параметром :ref:`Control.custom_maximum_size<class_Control_property_custom_maximum_size>`, якщо він заданий для відповідної осі). Це дозволяє йому спочатку розширюватися, а прокрутка починається лише після досягнення обмеження.
...
Якщо ``true``, колесо миші забезпечує горизонтальну прокрутку вікна, а при утриманні клавіші :kbd:`Shift` — вертикальну.

Якщо ``false`` (за замовчуванням), колесо миші забезпечує вертикальну прокрутку вікна, а при утриманні клавіші :kbd:`Shift` — горизонтальну.
