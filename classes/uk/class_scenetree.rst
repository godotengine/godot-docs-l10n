...
\ **Примітка:** Див. :ref:`change_scene_to_node()<class_SceneTree_method_change_scene_to_node>` для отримання детальної інформації про порядок операцій.
...
Змінює поточну сцену на наданий :ref:`Node<class_Node>`. Корисно, коли ви хочете налаштувати нову сцену перед зміною.

Повертає :ref:`@GlobalScope.OK<class_@GlobalScope_constant_OK>` у разі успіху, :ref:`@GlobalScope.ERR_INVALID_PARAMETER<class_@GlobalScope_constant_ERR_INVALID_PARAMETER>`, якщо ``node`` має значення ``null``, або :ref:`@GlobalScope.ERR_UNCONFIGURED<class_@GlobalScope_constant_ERR_UNCONFIGURED>`, якщо ``node`` вже знаходиться всередині дерева сцен.

\ **Примітка:** Операції відбуваються в такому порядку, коли викликається :ref:`change_scene_to_node()<class_SceneTree_method_change_scene_to_node>`:
...
2. В кінці кадру, попередньо поточна сцена, вже видалена з дерева, буде видалена (звільнена з пам'яті), а потім новий вузол сцени буде додано до дерева. :ref:`Node.get_tree()<class_Node_method_get_tree>` та :ref:`current_scene<class_SceneTree_property_current_scene>` повернуться до звичайної роботи.

Це гарантує, що обидві сцени не працюють одночасно, водночас звільняючи попередню сцену безпечним способом, подібним до :ref:`Node.queue_free()<class_Node_method_queue_free>`.
...
\ **Попередження:** Після використання цього методу **SceneTree** візьме на себе відповідальність за вузол і автоматично звільнить його під час наступної зміни сцени. Будь-які посилання на цей вузол стануть недійсними.
...
Змінює запущену сцену на новий екземпляр заданого :ref:`PackedScene<class_PackedScene>` (який має бути дійсним).
