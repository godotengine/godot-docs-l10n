...
Сигнал, що генерується при зміні стану присутності користувача.

\ **Примітка:** Цей сигнал не генерується під час запуску та завершення роботи програми. Розробникам слід виходити з того, що стан присутності користувача встановлюється під час запуску та втрачається під час завершення роботи програми.
...
Якщо ``true``, увімкнути динамічне регулювання фовеації. Перед тим, як скористатися цією функцією, необхідно ініціалізувати інтерфейс. Якщо функція увімкнена, фовеація автоматично регулюватиметься в діапазоні від мінімального рівня до :ref:`foveation_level<class_OpenXRInterface_property_foveation_level>`.
...
Рівень фовеації: від ``0`` (вимкнено) до ``3`` (високий). Щоб отримати доступ до цього параметра, спочатку потрібно ініціалізувати інтерфейс.
...
Якщо ``true``, увімкне зображення з пониженою роздільною здатністю та фовеацією, що може забезпечити підвищення продуктивності в Vulkan.
...
Повертає ``true``, якщо розширення OpenXR для визначення присутності користувача підтримується та увімкнено.

\ **Примітка:** Ця функція повертає дійсне значення лише після ініціалізації OpenXR.
...
Повертає ``true``, якщо система виявила присутність користувача в XR-досвіді.
