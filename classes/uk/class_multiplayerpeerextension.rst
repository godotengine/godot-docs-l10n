...
Викликається, коли підключений ``peer`` потрібно примусово відключити (див. :ref:`MultiplayerPeer.disconnect_peer()<class_MultiplayerPeer_method_disconnect_peer>`).
...
Викликається, коли :ref:`MultiplayerAPI<class_MultiplayerAPI>` потрібно надіслати пакет, причому ``buffer_size`` — це розмір двійкового ``buffer`` у байтах.
