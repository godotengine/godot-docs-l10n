...
:ref:`SkeletonModifier3D<class_SkeletonModifier3D>` для вирівнювання кісток уздовж :ref:`Path3D<class_Path3D>`. Рівномірність припасування залежить від параметра :ref:`Curve3D.bake_interval<class_Curve3D_property_bake_interval>`.

Якщо ви хочете, щоб :ref:`Path3D<class_Path3D>` прикріпився до конкретної кістки, рекомендується розмістити :ref:`ModifierBoneTarget3D<class_ModifierBoneTarget3D>` перед **SplineIK3D** у списку :ref:`SkeletonModifier3D<class_SkeletonModifier3D>` (дочірні елементи :ref:`Skeleton3D<class_Skeleton3D>`), а потім розмістити :ref:`Path3D<class_Path3D>` як дочірній елемент :ref:`ModifierBoneTarget3D<class_ModifierBoneTarget3D>`.

Поворот кістки визначається на основі методу ``Curve3D.get_point_tilt``.

Якщо суглоб кореневої кістки та початкова точка :ref:`Curve3D<class_Curve3D>` розділені, вважається, що між ними існує лінійний відрізок. Це означає, що вектор, спрямований до початкової точки :ref:`Curve3D<class_Curve3D>`, має пріоритет над найкоротшою точкою перетину вздовж :ref:`Curve3D<class_Curve3D>`.

Якщо кінцевий суглоб кістки виходить за межі довжини траєкторії, він згинається якомога ближче до кінцевої точки :ref:`Curve3D<class_Curve3D>`.

\ **Примітка:** Усі методи цього класу приймають параметр ``index``. Цей параметр визначає, який запис зі списку налаштувань повернути, якщо IK має кілька записів (наприклад, ``settings/<index>/root_bone_name``).
