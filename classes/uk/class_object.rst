...
Переопределите цей метод, щоб налаштувати поведінку методу :ref:`get()<class_Object_method_get>`. Повинен повертати значення вказаного ``property`` або ``null``, якщо ``property`` слід обробляти у звичайному режимі.

У поєднанні з :ref:`_set()<class_Object_private_method__set>` та :ref:`_get_property_list()<class_Object_private_method__get_property_list>` цей метод дозволяє визначати власні властивості, що особливо корисно для плагінів редактора.
...
\ **Примітка:** На відміну від інших віртуальних методів, цей метод викликається автоматично для кожного скрипта, який його перевизначає. Це означає, що базову реалізацію не слід викликати за допомогою ``super`` у GDScript або його еквівалентів в інших мовах. Спочатку буде викликано найнижчий підклас, а наступні виклики будуть здійснюватися вгору по ієрархії класів. Ланцюжок викликів зупиниться на першому класі, який поверне значення, відмінне від ``null``.

\ **Попередження:** Цей метод повинен бути :doc:`потокобезпечним <../tutorials/performance/thread_safe_apis>`, якщо його перевизначають. Інакше движок може аварійно завершити роботу під час спроби зберегти ресурс, що містить об’єкт.
...
Переопределите цей метод, щоб надати власний список додаткових властивостей, які має обробляти движок.

Повинен повертати список властивостей у вигляді масиву :ref:`Array<class_Array>` словників. Результат додається до масиву методу :ref:`get_property_list()<class_Object_method_get_property_list>` і повинен бути відформатований таким самим чином. Кожен :ref:`Dictionary<class_Dictionary>` повинен містити принаймні записи ``name`` та ``type``.

Ви можете використовувати :ref:`_property_can_revert()<class_Object_private_method__property_can_revert>` та :ref:`_property_get_revert()<class_Object_private_method__property_get_revert>`, щоб налаштувати значення за замовчуванням для властивостей, доданих цим методом.

У прикладі нижче відображається список чисел, показаних словами від ``ZERO`` до ``FIVE``, де ``number_count`` контролює розмір списку:
...
\ **Примітка:** Цей метод призначений для розширених цілей. Для більшості типових випадків використання мови сценаріїв пропонують простіші способи роботи з властивостями. Див. :ref:`@GDScript.@export<class_@GDScript_annotation_@export>`, :ref:`@GDScript.@export_enum<class_@GDScript_annotation_@export_enum>`, :ref:`@GDScript.@export_group<class_@GDScript_annotation_@export_group>` тощо. Якщо ви хочете налаштувати експортовані властивості, використовуйте :ref:`_validate_property()<class_Object_private_method__validate_property>`.

\ **Примітка:** Якщо скрипт об'єкта не є :ref:`@GDScript.@tool<class_@GDScript_annotation_@tool>`, цей метод не буде викликаний у редакторі.

\ **Примітка:** На відміну від інших віртуальних методів, цей метод викликається автоматично для кожного скрипта, який його перевизначає. Це означає, що базову реалізацію не слід викликати за допомогою ``super`` у GDScript або його еквівалентів в інших мовах. Спочатку буде викликано найнижчий підклас, а наступні виклики будуть здійснюватися вгору по ієрархії класів.
...
Ініціалізує ітератор. ``iter`` зберігає стан ітерації. Оскільки GDScript не підтримує передачу аргументів за посиланням, як обгортка використовується масив з одним елементом. Повертає ``true``, доки ітератор не досяг кінця.
...
\ **Примітка:** Уникайте зберігання стану ітератора у змінній-члені, замість цього використовуйте параметр ``iter``. Інакше ви не зможете повторно використовувати той самий екземпляр ітератора у вкладених циклах.

Дивіться також `онлайн-документацію <../tutorials/scripting/gdscript/gdscript_advanced.html#custom-iterators>`__.
...
Повертає ``true``, якщо для об’єкта було викликано методи :ref:`Node.queue_free()<class_Node_method_queue_free>` або :ref:`SceneTree.queue_delete()<class_SceneTree_method_queue_delete>`.

\ **Примітка:** Цей метод не повертає ``true`` для дочірніх елементів вузла, для якого було викликано :ref:`Node.queue_free()<class_Node_method_queue_free>`, навіть якщо вони будуть звільнені разом із батьківським елементом.
