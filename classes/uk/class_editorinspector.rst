...
Створює інспектор із тією ж конфігурацією, що й у панелі «Інспектор» редактора. При передачі об’єкта :ref:`LineEdit<class_LineEdit>` у параметр ``filter_line_edit`` інспектор буде фільтрувати його властивості на основі :ref:`LineEdit.text<class_LineEdit_property_text>` щоразу, коли генерується сигнал :ref:`LineEdit.text_changed<class_LineEdit_signal_text_changed>`.
...
Розгортає лише ті складні розділи, які містять властивість, яку можна скасувати (тобто не стандартну).
