...
Жоден з підресурсів не дублюється. Це корисно навіть у разі глибокого дублювання, коли всі масиви та словники дублюються, але при цьому продовжують вказувати на вихідні ресурси.
...
Унікальний ідентифікатор, пов'язаний зі сценою цього ресурсу. Якщо поле залишити порожнім, ідентифікатор генерується автоматично під час збереження цього ресурсу в :ref:`PackedScene<class_PackedScene>`. Якщо ресурс не знаходиться в сцені, це властивість за замовчуванням є порожнім.

\ **Примітка:** Під час збереження :ref:`PackedScene<class_PackedScene>`, якщо кілька ресурсів у одній сцені використовують однаковий ідентифікатор, лише найперший ресурс в ієрархії сцени зберігає оригінальний ідентифікатор. Іншим ресурсам присвоюються нові ідентифікатори за допомогою :ref:`generate_scene_unique_id()<class_Resource_method_generate_scene_unique_id>`.

\ **Примітка:** Встановлення цього властивості не викликає сигнал :ref:`changed<class_Resource_signal_changed>`.

\ **Попередження:** При встановленні ідентифікатор повинен складатися лише з літер, цифр та підкреслень. В іншому випадку операція завершиться невдачею, і за замовчуванням буде використано випадково згенерований ідентифікатор.
...
Копіює дані з ``resource`` у цей ресурс. Обидва ресурси повинні належати до одного й того самого класу.
