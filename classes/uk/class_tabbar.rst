...
\ **Примітка:** ``index`` — це значення в діапазоні ``0 .. tab_count - 1``.
...
Текст заголовка вкладки в ``index``.
...
Піктограма, що вказує місце, куди буде переміщено перетягнуту вкладку (див. :ref:`drag_to_rearrange_enabled<class_TabBar_property_drag_to_rearrange_enabled>`).
