...
Цей клас можна використовувати для виявлення сумісних пристроїв :ref:`UPNPDevice<class_UPNPDevice>` у локальній мережі та виконання команд на них, таких як управління відповідностями портів (для переадресації портів/обходу NAT) та запит IP-адреси локальної та віддаленої мережі. Зверніть увагу, що методи цього класу є синхронними та блокують потік, що їх викликає.

Щоб переадресувати конкретний порт (тут ``7777``, зверніть увагу, що як :ref:`discover()<class_UPNP_method_discover>`, так і :ref:`add_port_mapping()<class_UPNP_method_add_port_mapping>` можуть повертати помилки, які слід перевіряти):
...
Щоб закрити конкретний порт (наприклад, після того, як ви закінчили його використовувати):

::

    upnp.delete_port_mapping(port)

\ **Примітка:** Виявлення UPnP блокує поточний потік. Щоб виконати виявлення без блокування головного потоку, використовуйте :ref:`Thread<class_Thread>`\ s таким чином:

::

    # Видається після завершення налаштування відображення портів UPnP (незалежно від успіху чи невдачі).
    signal upnp_completed(error)

    # Замініть це на номер порту вашого сервера в діапазоні від 1024 до 65535.
    ...
    func _exit_tree():
     # Очікуємо завершення роботи потоку, щоб обробити вихід з гри під час його виконання.
     thread.wait_to_finish()

\ **Термінологія:** У контексті мереж UPnP «шлюз» (або «пристрій інтернет-шлюзу», скорочено IGD) означає мережеві пристрої, які дозволяють комп’ютерам у локальній мережі отримати доступ до Інтернету («глобальна мережа», WAN). Ці шлюзи часто також називають «маршрутизаторами».

\ **Підводні камені:**\ 

- Як пояснено вище, ці виклики є блокуючими і не повинні виконуватися в головному потоці, особливо з огляду на те, що вони можуть блокувати роботу на кілька секунд за раз. Використовуйте багатопотоковість!

- Мережеві технології є фізичними та хаотичними. Пакети губляться під час передачі або фільтруються, адреси, вільні порти та призначені відповідності змінюються, а пристрої можуть виходити з мережі або приєднуватися до неї в будь-який момент. Майте це на увазі, будьте пильними під час перевірки та обробки помилок і, якщо можливо, обробляйте їх коректно: додайте зрозумілий інтерфейс помилок, тайм-аути та обробку повторних спроб.

- Прив'язки портів можуть змінюватися (і видалятися) в будь-який момент, і віддалена/зовнішня IP-адреса шлюзу може змінюватися аналогічно. Вам слід розглянути можливість повторного запиту зовнішньої IP-адреси та спробувати періодично оновлювати/перезавантажувати прив'язку портів (наприклад, кожні 5 хвилин та під час мережевих збоїв).

- Не всі пристрої підтримують UPnP, а деякі користувачі вимикають підтримку UPnP. Вам потрібно вирішити цю проблему (наприклад, проінформувати користувача та попросити його вручну переадресувати порти, або додати альтернативні методи обходу NAT, такі як релейний/дзеркальний сервер, пробивання дірок у NAT, STUN/TURN тощо).

- Подумайте, що станеться у разі конфлікту прив’язок. Можливо, кілька користувачів у одній мережі захочуть грати у вашу гру одночасно, або інша програма використовує той самий порт. Зробіть порт настроюваним і оптимально вибирайте порт автоматично (при невдачі повторюйте спробу з іншим портом).

\ **Додаткова інформація:** Якщо ви хочете дізнатися більше про UPnP (а конкретно про пристрій інтернет-шлюзу (IGD) та протокол керування портами (PCP)), ` Вікіпедія <https://en.wikipedia.org/wiki/Universal_Plug_and_Play>`__ — це гарне місце для початку, специфікацію можна знайти на сайті `Open Connectivity Foundation <https://openconnectivity.org/developer/specifications/upnp-resources/upnp/>`__, а реалізація Godot базується на `клієнті MiniUPnP <https://github.com/miniupnp/miniupnp>`__.
