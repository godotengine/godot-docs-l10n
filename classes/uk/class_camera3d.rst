...
Встановлює режим проекції камери як «фрустум» (див. :ref:`PROJECTION_FRUSTUM<class_Camera3D_constant_PROJECTION_FRUSTUM>`) шляхом визначення параметра ``size``, ``offset``, а також площин обрізки ``z_near`` та ``z_far`` в одиницях простору світу. Параметр ``size`` визначає розмір ближньої площини — її ширину або висоту, залежно від значення :ref:`keep_aspect<class_Camera3D_property_keep_aspect>`. Див. також :ref:`frustum_offset<class_Camera3D_property_frustum_offset>`.
