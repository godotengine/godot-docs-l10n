...
Видає повідомлення про помилку та не дозволяє змінювати кількість стовпців під час виділення мишею.
...
Очищає дерево. При цьому видаляються всі елементи.

Виводить повідомлення про помилку і не дозволяє очистити дерево, якщо виклик відбувається під час виділення мишею.
...
Створює елемент у дереві та додає його як дочірній елемент ``parent``, який може бути або дійсним об’єктом типу :ref:`TreeItem<class_TreeItem>`, або ``null``.

Якщо ``parent`` дорівнює ``null``, батьківським елементом стане кореневий елемент, або ж новий елемент сам стане кореневим, якщо дерево порожнє.

Новий елемент буде ``index``-м дочірнім елементом батьківського елемента або останнім дочірнім елементом, якщо братів і сестер недостатньо.

Виводить повідомлення про помилку та повертає ``null``, якщо виклик відбувся під час виділення мишею або якщо ``parent`` не належить до цього дерева.
...
Повертає внутрішній елемент полотна, призначений для користувацького малювання. Див. :ref:`TreeItem.set_custom_draw_callback()<class_TreeItem_method_set_custom_draw_callback>`.

\ **Примітка:** Цей елемент полотна автоматично очищується під час кожного виклику малювання дерева.
...
:ref:`Color<class_Color>`, що використовується для відображення контуру виділення під час перетягування елементів, які можна розмістити лише «на» інших елементах.
