...
Цей :ref:`IKModifier3D<class_IKModifier3D>` потребує цільової точки. Він забезпечує детерміновані результати шляхом побудови площини, що проходить через кожне з'єднання та цільову точку, і знаходження точки перетину двох кіл (дисків у 3D).

Цей IK може обробляти скручування шляхом встановлення напрямку цільової точки. Якщо між кожним набором кісток є більше ніж одна кістка, їхні обертання ігноруються, а пряма лінія, що з'єднує кореневе-середнє та середнє-кінцеве з'єднання, розглядається як віртуальні кістки.
