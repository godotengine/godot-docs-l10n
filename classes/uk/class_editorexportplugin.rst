...
**EditorExportPlugin** автоматично викликаються щоразу, коли користувач експортує проект. Їх можна використовувати для модифікації сцен та ресурсів під час експорту проекту на основі встановлених :doc:`тегів функцій <../tutorials/export/feature_tags>`. Для кожного плагіна на початку процесу експорту викликається ``метод _export_begin``, а потім для кожного експортованого файлу викликається :ref:`_export_file()<class_EditorExportPlugin_private_method__export_file>`.

Зареєструйте **EditorExportPlugin**, створивши новий :ref:`EditorPlugin<class_EditorPlugin>` і викликавши його метод :ref:`EditorPlugin.add_export_plugin()<class_EditorPlugin_method_add_export_plugin>`.
...
Налаштуйте сцену. Якщо до неї внесено зміни, поверніть ту саму або нову сцену. В іншому випадку поверніть ``null``. Якщо повертається нова сцена, ви повинні самостійно звільнити пам’ять для старої.

Реалізація цього методу необхідна, якщо :ref:`_begin_customize_scenes()<class_EditorExportPlugin_private_method__begin_customize_scenes>` повертає ``true``.

\ **Примітка:** Щоб змінити змінну у вашій сцені, використовуйте анотацію ``@export`` під час її оголошення.
...
Ця функція викликається після створення проекту в Xcode, але до його компіляції.

\ **Примітка:** Підтримується лише в iOS та macOS.
...
Повертає хеш на основі переданої конфігурації (як для сцен, так і для ресурсів). Це дозволяє зберігати окремі кеші для різних конфігурацій експорту.

Реалізація цього методу є обов'язковою, якщо :ref:`_begin_customize_resources()<class_EditorExportPlugin_private_method__begin_customize_resources>` повертає ``true``.

\ **Примітка:** :ref:`_customize_resource()<class_EditorExportPlugin_private_method__customize_resource>` та :ref:`_customize_scene()<class_EditorExportPlugin_private_method__customize_scene>` не викликатимуться при модифікації скрипта **EditorExportPlugin**, якщо цей хеш також не зміниться.
